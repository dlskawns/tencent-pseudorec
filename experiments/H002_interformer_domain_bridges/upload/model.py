...
class _DomainBridge(nn.Module):
    """InterFormer-style low-rank cross-domain bridge (H002 transfer).

    Source: papers/unified_backbones/interformer_meta.md (arXiv:2411.09852).

    Given a pooled (B, D) summary of one domain's sequence-encoder output,
    project through a rank-r bottleneck and emit a (B, D) update gated by
    sigmoid(gate). Caller broadcasts the update to every position of the
    target domain's sequence.

    Initialization (paper-recommended near-off start):
        - gate scalar = ``gate_init`` (default -2 → sigmoid ≈ 0.12)
        - W_down xavier(gain=0.1), W_up zeros → at init the bridge emits 0
          regardless of gate, then both weights and gate co-train.
    """

    def __init__(self, d_model: int, rank: int = 4, gate_init: float = -2.0) -> None:
        super().__init__()
        ...
        nn.init.xavier_normal_(self.W_down.weight, gain=0.1)
        nn.init.zeros_(self.W_up.weight)

    def forward(self, pooled_src: torch.Tensor) -> torch.Tensor:
        # pooled_src: (B, D); returns (B, D) update.
        return torch.sigmoid(self.gate) * self.W_up(self.W_down(pooled_src))
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    H002 extension: when ``enable_inter_domain_bridges=True``, after
    per-domain Sequence Evolution and before Query Decoding, each domain's
    output is enriched with a gated low-rank summary of every other
    domain's pooled output (InterFormer-style bidirectional bridges).
    """
    ...
    def __init__(
        ...
        enable_inter_domain_bridges: bool = False,
        bridge_rank: int = 4,
        bridge_gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        self.num_sequences = num_sequences
        self.num_queries = num_queries
        self.num_ns = num_ns
        self.enable_inter_domain_bridges = enable_inter_domain_bridges
        ...
        # InterFormer cross-domain bridges (H002).
        # bridges[i][j] = bridge from source domain i → target domain j;
        # i == j slot is None (no self-bridge).
        if enable_inter_domain_bridges:
            self.bridges = nn.ModuleList([
                nn.ModuleList([
                    _DomainBridge(d_model, bridge_rank, bridge_gate_init)
    ...
    def forward(
        ...
        # 1.5. (H002) InterFormer cross-domain bridges.
        # Pool each domain's seq output mask-aware → (B, D), apply bridges,
        # broadcast updates back to every position of each target domain.
        if self.enable_inter_domain_bridges and self.bridges is not None:
            pooled_per_domain = []
            for i in range(S):
                h_i = next_seqs[i]                              # (B, L_i, D)
                m_i = next_masks[i]                             # (B, L_i), True=pad
                valid = (~m_i).unsqueeze(-1).to(h_i.dtype)      # (B, L_i, 1)
                denom = valid.sum(dim=1).clamp(min=1.0)         # (B, 1)
                pooled = (h_i * valid).sum(dim=1) / denom       # (B, D)
                pooled_per_domain.append(pooled)
            bridged = []
            for j in range(S):
                target = next_seqs[j]                           # (B, L_j, D)
                update = target.new_zeros(target.shape[0], target.shape[2])
                for i in range(S):
                    if i == j:
                        continue
                    bridge = self.bridges[i][j]
                    update = update + bridge(pooled_per_domain[i])
                bridged.append(target + update.unsqueeze(1))    # broadcast
            next_seqs = bridged
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H002: InterFormer inter-domain bridges
        enable_inter_domain_bridges: bool = False,
        bridge_rank: int = 4,
        bridge_gate_init: float = -2.0,
    ) -> None:
        ...
        self.bridge_gate_init = bridge_gate_init
        self.blocks = nn.ModuleList([
            MultiSeqHyFormerBlock(
                ...
                enable_inter_domain_bridges=enable_inter_domain_bridges,
