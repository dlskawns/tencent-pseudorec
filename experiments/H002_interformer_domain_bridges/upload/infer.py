...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    # Inference batch size: allow override via env (production has 2M+ rows;
    # bigger batch reduces CPU loading overhead).
    infer_batch_size = int(os.environ.get("INFER_BATCH_SIZE", cfg.get("batch_size", 256)))
    eval_ds.batch_size = infer_batch_size
    ...
    model = PCVRHyFormer(
        ...
        enable_inter_domain_bridges=cfg.get("enable_inter_domain_bridges", False),
        bridge_rank=cfg.get("bridge_rank", 4),
        bridge_gate_init=cfg.get("bridge_gate_init", -2.0),
    ).to(device)
    ...
    # num_workers: default 0 for safety (Taiji + CFS deadlocked at 8 during
    # training; 2 may be unsafe in eval container too). Set INFER_NUM_WORKERS=2
    # explicitly only after confirming no deadlock.
    infer_workers = int(os.environ.get("INFER_NUM_WORKERS", "0"))
    loader = DataLoader(
