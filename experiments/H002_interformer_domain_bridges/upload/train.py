...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--enable_inter_domain_bridges', action='store_true',
                        default=False,
                        help='H002: enable InterFormer-style cross-domain bridges '
                             'between per-domain seq encoders inside each '
                             'MultiSeqHyFormerBlock (low-rank, gated near-off init).')
    parser.add_argument('--bridge_rank', type=int, default=4,
                        help='Low-rank bottleneck dimension for inter-domain '
                             'bridges (paper-recommended 4).')
    parser.add_argument('--bridge_gate_init', type=float, default=-2.0,
                        help='Initial gate scalar (sigmoid space) for bridges; '
                             '-2 -> sigmoid ~0.12 (paper-recommended near-off).')
...
def main() -> None:
    ...
    model_args = {
        ...
        "enable_inter_domain_bridges": args.enable_inter_domain_bridges,
