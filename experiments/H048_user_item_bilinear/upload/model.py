...
class UserItemBilinearCross(nn.Module):
    """H048 — Explicit user × item bilinear cross at top of backbone.

    cross_i = user_repr_i ⊙ (W item_repr_i), output += sigmoid(gate) * Linear(cross).
    14 H 동안 user × item explicit bilinear interaction 0회. FM-style direct
    cross at top of trunk. Different from TWIN (item attends history) /
    DCN-V2 (within-side cross) / item-side cross (item only). NEW: user × item.
    """
    ...
    def forward(self, user_repr: torch.Tensor, item_repr: torch.Tensor) -> torch.Tensor:
        ...
        cross = user_repr * item_proj  # (B, D) element-wise multiply
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H048 — User × Item bilinear cross (residual ADD post-backbone)
        use_user_item_cross: bool = False,
        user_item_cross_gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        ...
        # H048 — User × Item bilinear cross
        self.use_user_item_cross = use_user_item_cross
        if use_user_item_cross:
            self.user_item_cross = UserItemBilinearCross(
                d_model=d_model,
                gate_init=user_item_cross_gate_init,
            )
            logging.info(
                f"H048 user-item bilinear cross enabled: gate_init={user_item_cross_gate_init}"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H048 — User × Item bilinear cross residual ADD (before TWIN to allow TWIN to see cross-aware base)
        if self.use_user_item_cross:
            item_ns = self.item_ns_tokenizer(inputs.item_int_feats)  # (B, num_item_ns, D)
            item_repr = item_ns.mean(dim=1)  # (B, D)
            output = output + self.user_item_cross(output, item_repr)
