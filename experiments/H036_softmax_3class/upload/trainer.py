...
class PCVRHyFormerRankingTrainer:
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if self.loss_type == 'ce_3class':
            # H036: 3-class CrossEntropy on raw label_type ∈ {0, 1, 2}.
            # raw_logits: (B, 3). Class 0 phantom (not in training data) → calibration.
            target_lt = device_batch['label_type'].long()  # (B,)
            loss = F.cross_entropy(raw_logits, target_lt)
    ...
    def _evaluate_step(
        ...
        if self.loss_type == 'ce_3class':
            # H036: convert (B, 3) → binary inference logit.
            # logit = log(P(c=2) / (P(c=0)+P(c=1))) = z_2 - logsumexp(z_0, z_1).
            logits = raw_logits[:, 2] - torch.logsumexp(raw_logits[:, :2], dim=-1)
        else:
            logits = raw_logits.squeeze(-1)
