...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            raw_logits, _ = model.predict(mi)
            # H036: dispatch inference logit by loss_type.
            if cfg.get('loss_type') == 'ce_3class':
                # 3-class softmax → binary inference logit = z_2 - logsumexp(z_0, z_1).
                inf_logit = raw_logits[:, 2] - torch.logsumexp(raw_logits[:, :2], dim=-1)
            else:
                inf_logit = raw_logits.squeeze(-1)
            probs = torch.sigmoid(inf_logit).detach().cpu().numpy()
