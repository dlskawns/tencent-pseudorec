...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.is_training:
            label_type_raw = (batch.column(self._col_idx['label_type']).fill_null(0)
                              .to_numpy(zero_copy_only=False).astype(np.int64))
            labels = (label_type_raw == 2).astype(np.int64)
        else:
            label_type_raw = np.zeros(B, dtype=np.int64)
        ...
        result = {
            ...
            'label_type': torch.from_numpy(label_type_raw),  # H036: raw label_type for 3-class CE
