#!/bin/bash
# Platform run.sh for H036_softmax_3class.
# Data-signal-driven (CLAUDE.md §0.5): negative class (label_type=1) 가
# heterogeneous — engagement-but-no-convert vs no-signal. Binary BCE 가 둘을
# 구분 못함. Mutation: action_num 1 → 3, BCE → CrossEntropy on raw label_type
# ∈ {0,1,2}. Class 0 phantom (training data 에 없음) → calibration 효과.
# H019 의 다른 모든 부분 byte-identical (TWIN, NS xattn, DCN-V2, gate=-2.0,
# top_k=64, seq 256, batch 1024).
#
...
export EXP_ID="H036_softmax_3class"
...
echo "[run.sh] mode=H036 3-class softmax CE on raw label_type (data signal: heterogeneous negative)"
...
echo "[run.sh] training complete (H036); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'loss_type=ce_3class action_num=3' indicator"
