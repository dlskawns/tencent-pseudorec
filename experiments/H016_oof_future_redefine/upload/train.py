...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--oof_split_type', type=str, default='random_user',
                        choices=['random_user', 'future_label_time'],
                        help='H016: OOF split rule. random_user (default, H010) = randomly '
                             'sampled 10% users. future_label_time = label_time >= quantile(label_time, '
                             '1 - oof_user_ratio). future_label_time → OOF distribution = pure future = '
                             'closer to platform eval. paired Δ baseline 깨짐 인정.')
...
def main() -> None:
    ...
    if args.use_label_time_split:
        ...
        train_loader, valid_loader, oof_loader, pcvr_dataset, split_meta = get_pcvr_data_v2(
            ...
            oof_split_type=args.oof_split_type,  # H016
        )
    else:
        logging.info(
