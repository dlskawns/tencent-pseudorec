...
class _RowFilter:
    ...
    __slots__ = ('oof_users_sorted', 'cutoff', 'target', 'oof_cutoff')

    def __init__(
        self,
        oof_users_sorted: np.ndarray,
        cutoff: float,
        target: str,
        oof_cutoff: Optional[float] = None,  # H016: time-based OOF cutoff
    ) -> None:
        ...
        self.target = target
        # H016: when oof_cutoff is not None, use time-based OOF (label_time >= oof_cutoff)
        # instead of user-based OOF. oof_users_sorted is then ignored (typically empty).
        self.oof_cutoff = float(oof_cutoff) if oof_cutoff is not None else None

    def __call__(self, batch: "pa.RecordBatch") -> np.ndarray:
        # H016 time-based OOF mode: ignore user list, use label_time only.
        if self.oof_cutoff is not None:
            lts = batch.column('label_time').to_numpy(zero_copy_only=False).astype(np.int64, copy=False)
            is_oof = lts >= self.oof_cutoff
            if self.target == 'oof':
                return is_oof
        ...
        # Default user-based OOF mode (H010 baseline).
...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for ci, dim, offset, vs in self._user_int_plan:
            col = batch.column(ci)
            if dim == 1:
                # Universal dim=1 handler: works regardless of column type
                # (scalar int, list, large_list, fixed_size_list, list_view, etc.).
                # Schema captured dim=1 so we always feed 1 value per row to the
                # model. For any list-shaped row we take the first non-null value.
                py = col.to_pylist()
        ...
        for ci, dim, offset, vs in self._item_int_plan:
            col = batch.column(ci)
            if dim == 1:
                # Universal dim=1 handler — same as user_int above.
                py = col.to_pylist()
                arr = np.empty(B, dtype=np.int64)
                for i, v in enumerate(py):
                    if v is None:
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    n_train_rgs_full = total_rgs - n_valid_rgs
    # tencent-cc2 patch: valid range must always start at the ORIGINAL train
    # boundary, not the (possibly reduced) n_train_rgs. Otherwise reducing
    # train_ratio inflates the valid set with the unused middle row groups.
    valid_start_rg = n_train_rgs_full

    n_train_rgs = n_train_rgs_full
    # train_ratio: use only the first N% of the training Row Groups.
    if train_ratio < 1.0:
        n_train_rgs = max(1, int(n_train_rgs_full * train_ratio))
        logging.info(f"train_ratio={train_ratio}: using {n_train_rgs} train Row Groups "
                     f"(out of {n_train_rgs_full} available; valid still uses last {n_valid_rgs})")
...
def split_parquet_by_label_time(
    ...
    oof_split_type: str = 'random_user',  # H016: 'random_user' (default) | 'future_label_time'
) -> Dict[str, Any]:
    ...
    # H016: OOF split mode dispatch.
    if oof_split_type == 'future_label_time':
        # Time-based OOF: rows with label_time >= oof_cutoff form the OOF set.
        # OOF distribution = pure future = closer to platform eval (presumably future).
        oof_cutoff = float(np.quantile(label_times, max(0.0, 1.0 - oof_user_ratio)))
        is_oof_all = label_times >= oof_cutoff
        ...
        # No user-based OOF set in this mode.
        oof_user_arr_sorted = np.array([], dtype=np.int64)
        oof_user_set_for_meta = set()
        n_oof_users = 0
        n_oof_pre = int(is_oof_all.sum())
        logging.info(f"split (H016 future_label_time): total_rows={n_total} "
                     f"unique_users={n_total_users} oof_cutoff={oof_cutoff:.0f} "
                     f"valid_cutoff={cutoff:.0f} n_oof={n_oof_pre}")
    else:
        # Default H010 mode: random user-based OOF.
        oof_cutoff = None
        ...
        for fi, f in enumerate(files, 1):
            ...
            for batch in pf.iter_batches(batch_size=pass2_batch):
                ...
                lts = (batch.column('label_time')
                       .to_numpy(zero_copy_only=False).astype(np.int64, copy=False))
                # H016: branch on oof split mode (oof_cutoff is None for default user-based mode).
                if oof_cutoff is not None:
                    is_oof = lts >= oof_cutoff
                else:
...
def get_pcvr_data_v2(
    ...
    **kwargs: Any,
    ...
    if meta_path.exists():
        try:
            ...
            same = (
                ...
                and str(cached.get('oof_split_type', 'random_user')) == oof_split_type  # H016: invalidate cache on mode change
    ...
    if cached is not None:
        ...
        # H016: time-based OOF cutoff (None for default user-based mode).
        oof_cutoff = cached.get('oof_cutoff', None)
        if oof_cutoff is not None:
            oof_cutoff = float(oof_cutoff)
    else:
        ...
        if oof_split_type == 'future_label_time':
            ...
            if len(non_oof_lt) == 0:
                raise RuntimeError("OOF holdout consumed all rows; lower oof_user_ratio")
            cutoff = float(np.quantile(non_oof_lt, max(0.0, 1.0 - valid_ratio)))
            oof_users_sorted = np.array([], dtype=np.int64)
            n_oof_users = 0
        else:
            # Default H010 random user-based OOF.
            oof_cutoff = None
        ...
        meta_to_cache: Dict[str, Any] = {
            ...
            'valid_ratio': float(valid_ratio),
            'oof_user_ratio': float(oof_user_ratio),
            'oof_split_type': oof_split_type,  # H016
            'oof_cutoff': oof_cutoff,           # H016 (None for user-based mode)
            'label_time_cutoff': cutoff,
            'n_total': n_total,
            'n_total_users': n_total_users,
            'n_oof_users': int(n_oof_users),
    ...
    # H016: pass oof_cutoff to _RowFilter (None for default user-based mode).
    train_filter = _RowFilter(oof_users_sorted, cutoff, 'train', oof_cutoff=oof_cutoff)
    valid_filter = _RowFilter(oof_users_sorted, cutoff, 'valid', oof_cutoff=oof_cutoff)
    oof_filter = _RowFilter(oof_users_sorted, cutoff, 'oof', oof_cutoff=oof_cutoff)
