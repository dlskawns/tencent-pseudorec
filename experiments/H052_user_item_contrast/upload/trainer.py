...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        contrast_lambda: float = 0.0,    # H052 — 0.0 → off (H019 byte-identical)
        contrast_temperature: float = 0.1,    # H052 — InfoNCE temperature
        ...
        self.contrast_lambda: float = contrast_lambda    # H052
        self.contrast_temperature: float = contrast_temperature    # H052
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H052: model returns (logits, user_repr, item_repr) tuple when use_user_item_contrast.
        if isinstance(model_out, tuple):
            raw_logits, user_repr, item_repr = model_out
        else:
            raw_logits = model_out
            user_repr, item_repr = None, None
        ...
        # H052 — User-Item InfoNCE: positive = (user_i, item_i) same row,
        # negatives = (user_i, item_j) other rows in batch (in-batch negatives).
        # forces user_repr to be discriminative about its own item vs others.
        if user_repr is not None and self.contrast_lambda > 0:
            ...
            loss = loss + self.contrast_lambda * contrast_loss
