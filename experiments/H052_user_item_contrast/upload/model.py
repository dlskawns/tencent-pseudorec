...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_user_item_contrast: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H052 — User-Item Contrastive auxiliary (no params, just forward tuple flag)
        self.use_user_item_contrast = use_user_item_contrast
        if use_user_item_contrast:
            logging.info(
                f"H052 user-item contrastive aux enabled: forward returns (logits, user_repr, item_repr)"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        H052: returns (logits, user_repr, item_repr) tuple when
        use_user_item_contrast=True. Otherwise returns logits only (H019 byte-identical).
        """
        ...
        # H052 — return representations for InfoNCE in trainer.
        if self.use_user_item_contrast:
            item_ns = self.item_ns_tokenizer(inputs.item_int_feats)  # (B, num_item_ns, D)
            item_repr = item_ns.mean(dim=1)  # (B, D)
            return logits, output, item_repr
        return logits
