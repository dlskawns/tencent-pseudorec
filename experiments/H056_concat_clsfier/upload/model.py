...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H056 — Concat-input classifier (item_repr concat to clsfier input, NO residual stacking)
        use_concat_clsfier: bool = False,
    ) -> None:
        super().__init__()
        ...
        # Classifier (H056: optional concat input — first Linear takes 2*d_model when enabled)
        self.use_concat_clsfier = use_concat_clsfier
        clsfier_in_dim = (2 * d_model) if use_concat_clsfier else d_model
        ...
        if use_concat_clsfier:
            logging.info(
                f"H056 concat-input clsfier enabled: input dim 2*{d_model}={clsfier_in_dim} "
                f"(output + item_repr concat). NO residual stacking on output (avoids H048 fail mode)."
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H056 — Concat-input classifier: append item_repr to clsfier input (NO output residual ADD).
        if self.use_concat_clsfier:
            item_ns = self.item_ns_tokenizer(inputs.item_int_feats)  # (B, num_item_ns, D)
            item_repr = item_ns.mean(dim=1)  # (B, D)
            clsfier_input = torch.cat([output, item_repr], dim=-1)  # (B, 2D)
        else:
            clsfier_input = output
        logits = self.clsfier(clsfier_input)  # (B, action_num)
        return logits
