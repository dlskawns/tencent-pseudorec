...
class TWINBlock(nn.Module):
    ...
    H060: optional learnable positional embedding for top-K positions added
    BEFORE ESU MHA. Helps ESU distinguish 1st-top vs Kth-top events (currently
    "bag of top-K" with no rank info). TWIN-internal mutation, single residual preserved.
    """

    def __init__(self, d_model: int, num_heads: int = 4, top_k: int = 64,
                 use_pos_emb: bool = False) -> None:
        super().__init__()
        self.d_model = d_model
        self.top_k = top_k
        self.esu = nn.MultiheadAttention(d_model, num_heads, batch_first=True)
        self.norm = nn.LayerNorm(d_model)
        # H060 — learnable positional embedding for top-K positions (rank-aware).
        self.use_pos_emb = use_pos_emb
        if use_pos_emb:
            self.pos_emb = nn.Embedding(top_k, d_model)
    ...
    def forward(
        ...
        # H060: add learnable positional embedding for top-K rank positions.
        if self.use_pos_emb:
            pos_ids = torch.arange(K, device=history.device)
            pos_emb = self.pos_emb(pos_ids)  # (K, D)
            topk_history = topk_history + pos_emb.unsqueeze(0)  # broadcast (B, K, D)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H060 — TWIN ESU positional embedding for top-K (rank-aware MHA)
        use_twin_esu_pos_emb: bool = False,
    ) -> None:
        super().__init__()
        ...
        if use_twin_retrieval:
            self.twin_blocks = nn.ModuleDict({
                f'twin_{domain}': TWINBlock(
                    ...
                    top_k=twin_top_k,
                    use_pos_emb=use_twin_esu_pos_emb,  # H060
                )
