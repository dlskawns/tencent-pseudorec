...
class TwinRetrievalAggregator(nn.Module):
    ...
    H057: optional attention-weighted aggregation (per-user per-domain importance)
    instead of uniform mean. Same single TWIN residual ADD, internal change only.
    """

    def __init__(self, d_model: int, gate_init: float = -2.0, use_attn: bool = False) -> None:
        super().__init__()
    ...
    def forward(self, per_domain_states: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_attn:
            # H057: per-user per-domain attention weight (softmax over domains).
            attn_logits = self.attn_weights(stacked).squeeze(-1)  # (B, num_domains)
            weights = torch.softmax(attn_logits, dim=-1)  # (B, num_domains)
            pooled = (stacked * weights.unsqueeze(-1)).sum(dim=1)  # (B, D)
        else:
            pooled = stacked.mean(dim=1)  # (B, D) — H019 uniform mean
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H057 — TWIN aggregator attention (mean → per-user per-domain weighted)
        use_twin_attn_aggregator: bool = False,
    ) -> None:
        super().__init__()
