...
class CrossDomainBridge(nn.Module):
    """H045 — Cross-domain attention bridge over per-domain seq encoder outputs.

    Per-domain seq tokens (4 × (B, T, D)) → masked mean-pool per domain →
    stack (B, 4, D) → MultiheadAttention across 4 domain tokens (each domain
    attends to the other 3) → mean over domain dim → (B, D) → Linear+LN+gated
    residual ADD.

    Attacks §3.5 4-domain disjoint-but-overlapping vocab (jaccard 0.7~10%):
    TWIN aggregator does mean-pool only, no explicit cross-domain feature
    transfer. CrossDomainBridge adds explicit attention bridging.
    """

    def __init__(self, d_model: int, num_heads: int = 4, gate_init: float = -2.0) -> None:
        super().__init__()
    ...
    def _masked_mean(tokens: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Masked mean over T dim. tokens: (B, T, D), mask: (B, T) True=pad."""
        valid = (~mask).float().unsqueeze(-1)
        sum_valid = valid.sum(dim=1).clamp(min=1.0)
        return (tokens * valid).sum(dim=1) / sum_valid
    ...
    def forward(
        ...
        per_domain = [
            self._masked_mean(t, m) for t, m in zip(seq_tokens_list, seq_masks_list)
        ]
        stacked = torch.stack(per_domain, dim=1)  # (B, 4, D)
        attended, _ = self.attn(stacked, stacked, stacked, need_weights=False)
        attended = self.attn_ln(attended + stacked)
        pooled = attended.mean(dim=1)  # (B, D)
        bridged = self.out_ln(self.proj(pooled))
        gate = torch.sigmoid(self.bridge_gate)
        return gate * bridged
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H045 — Cross-domain bridge attention (per-domain encoder output cross-talk)
        use_cross_domain_bridge: bool = False,
        cross_domain_num_heads: int = 4,
        cross_domain_gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        ...
        # H045 — Cross-domain bridge attention
        self.use_cross_domain_bridge = use_cross_domain_bridge
        if use_cross_domain_bridge:
            self.cross_domain_bridge = CrossDomainBridge(
                d_model=d_model,
                num_heads=cross_domain_num_heads,
                gate_init=cross_domain_gate_init,
            )
            logging.info(
                f"H045 cross-domain bridge enabled: "
                f"num_heads={cross_domain_num_heads} gate_init={cross_domain_gate_init}"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H045 — Cross-domain bridge residual ADD (before TWIN to allow TWIN to see bridge-aware base)
        if self.use_cross_domain_bridge:
            output = output + self.cross_domain_bridge(seq_tokens_list, seq_masks_list)
