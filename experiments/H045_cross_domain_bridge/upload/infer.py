...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    model = PCVRHyFormer(
        ...
        use_cross_domain_bridge=cfg.get("use_cross_domain_bridge", False),
        cross_domain_num_heads=cfg.get("cross_domain_num_heads", 4),
        cross_domain_gate_init=cfg.get("cross_domain_gate_init", -2.0),
    ).to(device)
