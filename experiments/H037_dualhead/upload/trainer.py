...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        dualhead_lambda: float = 0.5,    # H037
        ...
        self.dualhead_lambda: float = dualhead_lambda    # H037
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if self.loss_type == 'dualhead':
            # H037: 2 binary heads with anti-correlated targets.
            # raw_logits: (B, 2). col 0 = head_pos (label_type==2), col 1 = head_neg (label_type==1).
            logit_pos = raw_logits[:, 0]
            logit_neg = raw_logits[:, 1]
            bce_pos = F.binary_cross_entropy_with_logits(logit_pos, label)
            bce_neg = F.binary_cross_entropy_with_logits(logit_neg, 1.0 - label)
            loss = bce_pos + self.dualhead_lambda * bce_neg
    ...
    def _evaluate_step(
        ...
        if self.loss_type == 'dualhead':
            # H037: head_pos (col 0) is the main inference logit.
            logits = raw_logits[:, 0]
        else:
            logits = raw_logits.squeeze(-1)
