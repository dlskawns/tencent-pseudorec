...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--loss_type', type=str, default='bce',
                        choices=['bce', 'focal', 'dualhead'],
                        help='Loss type: bce = BCEWithLogits, focal = Focal Loss, '
                             'dualhead = H037 2 binary heads (head_pos label_type==2 + '
                             'head_neg label_type==1) with action_num=2.')
    parser.add_argument('--dualhead_lambda', type=float, default=0.5,
                        help='H037: weight for head_neg BCE (loss = BCE_pos + λ·BCE_neg).')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        dualhead_lambda=args.dualhead_lambda,    # H037
