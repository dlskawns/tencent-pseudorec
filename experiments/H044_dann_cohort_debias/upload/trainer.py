...
class _GradReverse(torch.autograd.Function):
    """H044 — Gradient reversal layer (Ganin & Lempitsky 2015 DANN).

    Forward: identity. Backward: multiply gradient by -lambda.
    Used to make backbone *forget* a signal (cohort/timestamp) by training
    a discriminator on it but reversing the gradient that flows back into
    the shared backbone — adversarial direction.
    """
...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        dann_lambda: float = 0.0,   # H044 — 0.0 → off (H038 byte-identical)
        ...
        self.dann_lambda: float = dann_lambda    # H044
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if self.loss_type == 'aux_timestamp':
            ...
            # H044: dann_lambda > 0 → gradient reversal on aux path → backbone
            # *forgets* timestamp (cohort signal). Adversarial direction.
