...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--dann_lambda', type=float, default=0.0,
                        help='H044: gradient reversal strength on aux timestamp path '
                             '(0.0 → off, H038 byte-identical). dann_lambda > 0 → '
                             'backbone *forgets* timestamp signal (adversarial DANN direction).')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        dann_lambda=args.dann_lambda,    # H044
