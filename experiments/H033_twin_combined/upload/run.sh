#!/bin/bash
# Platform run.sh for H033_twin_combined.
# Stacking sub-H of H019: H020 (learnable GSU) + H021 (per-domain top_k)
# 동시 적용. 두 직교 axis (scoring quality + scoring quantity) 동시 mutation
# 으로 stacking effect 검증. control = H019 (paired Δ vs cloud actual 0.839674).
...
export EXP_ID="H033_twin_combined"
...
echo "[run.sh] mode=H033 TWIN combined (H020 learnable_gsu + H021 per-domain top_k stacked)"
...
echo "[run.sh] training complete (H033); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'H033 TWIN combined' log line"
