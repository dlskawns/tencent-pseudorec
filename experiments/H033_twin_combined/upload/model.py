...
class TWINBlock(nn.Module):
    ...
    H019: GSU = parameter-free inner product on backbone embedding space.
    H020: GSU = learnable projection (W_q, W_k: Linear(d_model, d_model//4))
          before inner product (paper-faithful form, Tencent 2024 RecSys).
    """
    ...
    def forward(
        ...
        B, L, D = history.shape
        K = min(self.top_k, L)
        # GSU — score per history token vs candidate.
        if self.learnable_gsu:
            ...
            # H019 — parameter-free inner product on backbone embedding space
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H033 — twin_top_k accepts int (uniform) OR dict {seq_a, seq_b, seq_c, seq_d → int} (per-domain)
        use_twin_retrieval: bool = False,
        twin_top_k=64,
        twin_num_heads: int = 4,
        twin_gate_init: float = -2.0,
        # H020 — learnable GSU projection (paper-faithful form)
        twin_learnable_gsu: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H020 — learnable_gsu flag passed to TWINBlock
        # H033 — H020 (learnable_gsu) + H021 (per-domain top_k) stacked
        self.use_twin_retrieval = use_twin_retrieval
        if use_twin_retrieval:
            # H033: accept int (uniform, H019 backward compat) OR dict (per-domain).
            if isinstance(twin_top_k, int):
                top_k_per_domain = {d: twin_top_k for d in ('seq_a', 'seq_b', 'seq_c', 'seq_d')}
            else:
                top_k_per_domain = dict(twin_top_k)
                missing = set(('seq_a', 'seq_b', 'seq_c', 'seq_d')) - set(top_k_per_domain.keys())
                if missing:
                    raise ValueError(
                        f"twin_top_k dict missing domain(s): {missing}. "
                        f"Expected keys: {{seq_a, seq_b, seq_c, seq_d}}."
                    )
            self.twin_blocks = nn.ModuleDict({
                f'twin_{domain}': TWINBlock(
                    d_model=d_model,
                    num_heads=twin_num_heads,
                    top_k=int(top_k_per_domain[domain]),
                    learnable_gsu=twin_learnable_gsu,
                )
            ...
            logging.info(
                f"H033 TWIN combined enabled: per-domain K = "
                f"seq_a={top_k_per_domain['seq_a']} seq_b={top_k_per_domain['seq_b']} "
                f"seq_c={top_k_per_domain['seq_c']} seq_d={top_k_per_domain['seq_d']} "
                f"learnable_gsu={twin_learnable_gsu} num_heads={twin_num_heads} "
                f"gate_init={twin_gate_init}"
            )
        else:
