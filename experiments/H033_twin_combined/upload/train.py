...
def _parse_twin_top_k_arg(args) -> object:
    """H033 — parse --twin_top_k_per_domain string into dict, or fall back
    to --twin_top_k int (H019 backward compat).

    Returns:
        int (uniform) OR dict {seq_a, seq_b, seq_c, seq_d → int} (per-domain).
    """
    if not args.twin_top_k_per_domain:
        return args.twin_top_k       # H019 backward compat
    parts = [int(x.strip()) for x in args.twin_top_k_per_domain.split(',')]
    if len(parts) != 4:
        raise SystemExit(
            f"--twin_top_k_per_domain must be 4 comma-separated ints "
            f"(seq_a,seq_b,seq_c,seq_d), got {len(parts)}: "
            f"{args.twin_top_k_per_domain!r}"
        )
    parsed = {d: k for d, k in zip(('seq_a', 'seq_b', 'seq_c', 'seq_d'), parts)}
    logging.info(f"H033 enabled: per-domain K = {parsed}")
    return parsed
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--twin_learnable_gsu', action='store_true', default=False,
                        help='H020: enable learnable GSU projection (W_q, W_k: '
                             'Linear(d_model, d_model//4, bias=False)). '
                             'Paper-faithful form (Tencent 2024 RecSys). '
                             'Default False = H019 parameter-free inner product.')
    parser.add_argument('--twin_top_k_per_domain', type=str, default=None,
                        help='H021/H033: comma-separated 4-int '
                             '(seq_a,seq_b,seq_c,seq_d) per-domain K, '
                             'e.g. "64,64,64,96". Overrides --twin_top_k if set. '
                             'Default None = H019 uniform --twin_top_k value.')
...
def main() -> None:
    ...
    model_args = {
        ...
        # H019 — TWIN long-seq retrieval
        # H033 — per-domain top_k 파싱
        "use_twin_retrieval": args.use_twin_retrieval,
        "twin_top_k": _parse_twin_top_k_arg(args),
        "twin_num_heads": args.twin_num_heads,
        "twin_gate_init": args.twin_gate_init,
        # H020 — learnable GSU projection
        "twin_learnable_gsu": args.twin_learnable_gsu,
    }
