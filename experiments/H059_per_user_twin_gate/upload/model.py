...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H059 — Per-user adaptive TWIN gate (replaces scalar twin_gate)
        use_per_user_twin_gate: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H059 — Per-user adaptive TWIN gate (per-sample sigmoid based on backbone output)
        self.use_per_user_twin_gate = use_per_user_twin_gate
        if use_per_user_twin_gate:
            self.twin_per_user_gate_proj = nn.Linear(d_model, 1)
            nn.init.normal_(self.twin_per_user_gate_proj.weight, std=0.02)
            nn.init.constant_(self.twin_per_user_gate_proj.bias, float(twin_gate_init))
            logging.info(
                f"H059 per-user TWIN gate enabled: per-sample sigmoid(Linear(output)), bias init={twin_gate_init} (≈0.12)"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H059 — replace aggregator scalar twin_gate with per-sample gate from backbone output
        if self.use_twin_retrieval:
            twin_residual = self._compute_twin_residual(inputs, seq_tokens_list, seq_masks_list)
            if self.use_per_user_twin_gate:
                scalar_gate = torch.sigmoid(self.twin_aggregator.twin_gate)
                per_user_gate = torch.sigmoid(self.twin_per_user_gate_proj(output))  # (B, 1)
                twin_residual = (twin_residual / scalar_gate.clamp(min=1e-6)) * per_user_gate
