...
# ═══════════════════════════════════════════════════════════════════════════════
# H031 — Item13UserCrossBlock (fid-level explicit cross, EDA-driven)
# ═══════════════════════════════════════════════════════════════════════════════
# Motivation (eda/out/item_int_signal_audit.json):
#   item_int_feats_13 (vs=10) 단일 GBDT 5-fold AUC 0.6561, i13=4 → 64% positive
#   (+51pt lift). 9 H 모두 RankMixerNSTokenizer block-level 합산 → dilute.
#
# Mechanism: item_13 dedicated 32-dim emb + user_int{1,97,58,49,95} (모두 vs ≤ 6)
# dedicated 16-dim emb. Pair-wise outer-product cross → mean over 5 pairs →
# Linear → LayerNorm → (B, d_model) cross_state.
#
# Integration: residual ADD post-backbone (output += sigmoid(gate) * cross_state).
# §10.10 mandate: gate init = sigmoid(-2.0) ≈ 0.12 (near-off start, InterFormer
# pattern). num_ns count 변경 없음 → d_model % T 제약 안전.
#
# Offsets are computed from data/schema.json (eda/out/item_int_signal_audit.json
# 의 'h031_picked_5_fids' + item_int order) and hard-coded as defaults.


class Item13UserCrossBlock(nn.Module):
    """Fid-level explicit cross between item_int_feats_13 and 5 small-vocab
    user_int features. Output = (B, d_model) residual state + sigmoid gate.

    Default offsets (data/schema.json):
        item_13 → item_int_feats[:, 27], vs=10
        user_int_1  → user_int_feats[:, 0],   vs=6
        user_int_49 → user_int_feats[:, 17],  vs=4
        user_int_58 → user_int_feats[:, 26],  vs=4
        user_int_95 → user_int_feats[:, 219], vs=5
        user_int_97 → user_int_feats[:, 221], vs=5
    """

    def __init__(
        self,
        d_model: int,
        item13_offset: int = 27,
        item13_vs: int = 10,
        user_specs: Optional[List[Tuple[int, int]]] = None,
        item_emb_dim: int = 32,
        user_emb_dim: int = 16,
        cross_dim: int = 32,
        gate_init: float = -2.0,
        ...
        if user_specs is None:
            user_specs = [(0, 6), (221, 5), (26, 4), (17, 4), (219, 5)]
        self.item13_offset = item13_offset
        self.user_specs = user_specs
        self.item13_emb = nn.Embedding(item13_vs + 1, item_emb_dim, padding_idx=0)
        self.user_embs = nn.ModuleList([
            nn.Embedding(vs + 1, user_emb_dim, padding_idx=0) for _, vs in user_specs
        ])
        self.cross_projs = nn.ModuleList([
            nn.Linear(item_emb_dim * user_emb_dim, cross_dim) for _ in user_specs
        ])
        self.out_proj = nn.Linear(cross_dim, d_model)
        self.out_ln = nn.LayerNorm(d_model)
        self.cross_gate = nn.Parameter(torch.tensor(float(gate_init)))
        nn.init.xavier_normal_(self.item13_emb.weight.data)
        self.item13_emb.weight.data[0, :] = 0
    ...
    def forward(
        ...
        item13_idx = item_int_feats[:, self.item13_offset].long().clamp_min(0)
        item13_e = self.item13_emb(item13_idx)
        cross_outs: List[torch.Tensor] = []
        for (off, _vs), user_emb_layer, cross_proj in zip(
            self.user_specs, self.user_embs, self.cross_projs
        ):
            user_idx = user_int_feats[:, off].long().clamp_min(0)
            user_e = user_emb_layer(user_idx)
            outer = (item13_e.unsqueeze(-1) * user_e.unsqueeze(1)).flatten(1)
            cross_outs.append(cross_proj(outer))
        cross = torch.stack(cross_outs, dim=1).mean(dim=1)
        cross_state = self.out_ln(self.out_proj(cross))
        gate = torch.sigmoid(self.cross_gate)
        return cross_state, gate
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H031 — Item13 × user_int explicit fid-level cross (residual ADD post-backbone)
        use_item13_user_cross: bool = False,
        item13_offset: Optional[int] = None,
        item13_vs: Optional[int] = None,
        item13_user_specs: Optional[List[Tuple[int, int]]] = None,
        item13_emb_dim: int = 32,
        item13_user_emb_dim: int = 16,
        item13_cross_dim: int = 32,
        item13_gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        ...
        # H031 — Item13 × user_int explicit fid-level cross block
        self.use_item13_user_cross = use_item13_user_cross
        if use_item13_user_cross:
            if item13_offset is None or item13_vs is None or not item13_user_specs:
                raise ValueError(
                    f"H031 use_item13_user_cross=True requires runtime schema args: "
                    f"item13_offset={item13_offset}, item13_vs={item13_vs}, "
                    f"item13_user_specs={item13_user_specs}. Pass from train.py "
                    f"after schema load (cloud rebuilds schema → vs hardcode UNSAFE)."
                )
            self.item13_user_cross = Item13UserCrossBlock(
                d_model=d_model,
                item13_offset=item13_offset,
                item13_vs=item13_vs,
                user_specs=item13_user_specs,
                item_emb_dim=item13_emb_dim,
                user_emb_dim=item13_user_emb_dim,
                cross_dim=item13_cross_dim,
                gate_init=item13_gate_init,
            )
            logging.info(
                f"H031 Item13UserCrossBlock enabled: item13_offset={item13_offset} "
                f"item13_vs={item13_vs} user_specs={item13_user_specs} "
                f"item_emb_dim={item13_emb_dim} user_emb_dim={item13_user_emb_dim} "
                f"cross_dim={item13_cross_dim} gate_init={item13_gate_init}"
            )
        else:
            self.item13_user_cross = None
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H031 — residual ADD of fid-level explicit cross (gated, init ≈ 0.12)
        if self.item13_user_cross is not None:
            cross_state, gate = self.item13_user_cross(
                inputs.user_int_feats, inputs.item_int_feats
            )
            output = output + gate * cross_state
