...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    model = PCVRHyFormer(
        ...
        use_item13_user_cross=cfg.get("use_item13_user_cross", False),
        item13_offset=cfg.get("item13_offset"),
        item13_vs=cfg.get("item13_vs"),
        item13_user_specs=[tuple(p) for p in cfg.get("item13_user_specs", [])] or None,
        item13_emb_dim=cfg.get("item13_emb_dim", 32),
        item13_user_emb_dim=cfg.get("item13_user_emb_dim", 16),
        item13_cross_dim=cfg.get("item13_cross_dim", 32),
        item13_gate_init=cfg.get("item13_gate_init", -2.0),
    ).to(device)
