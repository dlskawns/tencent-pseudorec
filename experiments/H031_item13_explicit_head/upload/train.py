...
def parse_args() -> argparse.Namespace:
    ...
    # H031 — Item13 × user_int explicit fid-level cross
    parser.add_argument('--use_item13_user_cross', action='store_true', default=False,
                        help='H031: enable Item13UserCrossBlock — item_int_feats_13 (vs=10) '
                             'dedicated emb × 5 small-vocab user_int (fids 1, 49, 58, 95, 97) '
                             'outer-product cross → mean over pairs → residual ADD post-backbone. '
                             'Gate init = sigmoid(-2) ≈ 0.12 per CLAUDE.md §10.10. '
                             'EDA: eda/out/item_int_signal_audit.json.')
    parser.add_argument('--item13_emb_dim', type=int, default=32,
                        help='H031: item_13 dedicated embedding dim. Effective with --use_item13_user_cross.')
    parser.add_argument('--item13_user_emb_dim', type=int, default=16,
                        help='H031: per-user_int dedicated embedding dim. Effective with --use_item13_user_cross.')
    parser.add_argument('--item13_cross_dim', type=int, default=32,
                        help='H031: pair-wise cross output dim before mean+project. Effective with --use_item13_user_cross.')
    parser.add_argument('--item13_gate_init', type=float, default=-2.0,
                        help='H031: cross_gate init (sigmoid(-2.0) ≈ 0.12 per InterFormer §10.10).')
...
def main() -> None:
    ...
    # H031 — runtime extraction of (offset, vs) for Item13UserCrossBlock.
    # CRITICAL: cloud rebuilds schema from full data (make_schema.py) → vs may
    # differ from demo_1000 hardcode. Reading runtime schema avoids CUDA index
    # out-of-bounds (SIGABRT) seen in initial H031 launch.
    item13_offset = None
    item13_vs = None
    for fid, off, length in pcvr_dataset.item_int_schema.entries:
        if fid == 13:
            item13_offset = off
            item13_vs = pcvr_dataset.item_int_vocab_sizes[off]
            break
    user_int_target_fids = [1, 49, 58, 95, 97]  # H031 picks per EDA item_int_signal_audit.json
    item13_user_specs = []
    user_fid_offset = {fid: off for fid, off, _ in pcvr_dataset.user_int_schema.entries}
    for fid in user_int_target_fids:
        if fid in user_fid_offset:
            off = user_fid_offset[fid]
            vs = pcvr_dataset.user_int_vocab_sizes[off]
            item13_user_specs.append((off, vs))
    if args.use_item13_user_cross:
        if item13_offset is None or item13_vs is None or len(item13_user_specs) == 0:
            raise ValueError(
                f"H031 --use_item13_user_cross enabled but schema missing required fids. "
                f"item13_offset={item13_offset}, item13_vs={item13_vs}, "
                f"user_int_specs_count={len(item13_user_specs)}")
        logging.info(
            f"H031 runtime spec: item_13 offset={item13_offset} vs={item13_vs}, "
            f"user_int_specs={item13_user_specs}")
    ...
    model_args = {
        ...
        # H031 — Item13 × user_int explicit fid-level cross
        "use_item13_user_cross": args.use_item13_user_cross,
        "item13_offset": item13_offset,
        "item13_vs": item13_vs,
        "item13_user_specs": item13_user_specs,
        "item13_emb_dim": args.item13_emb_dim,
        "item13_user_emb_dim": args.item13_user_emb_dim,
        "item13_cross_dim": args.item13_cross_dim,
        "item13_gate_init": args.item13_gate_init,
    }
    ...
    # H023 — diagnostic prints around OOF eval (debug H022 OOF=N/A issue).
    print(f"[H023 DEBUG] oof_loader is None: {oof_loader is None}", flush=True)
    print(f"[H023 DEBUG] early_stopping.best_model is None: {trainer.early_stopping.best_model is None}", flush=True)
    print(f"[H023 DEBUG] split_meta: {split_meta}", flush=True)
    if oof_loader is not None and trainer.early_stopping.best_model is not None:
        ...
        try:
            ...
            print(f"OOF LogLoss: {oof_ll:.4f}", flush=True)
        except Exception as e:
            import traceback
            print(f"[H023 DEBUG] OOF eval RAISED: {type(e).__name__}: {e}", flush=True)
            traceback.print_exc()
            logging.warning(f"OOF eval failed: {e}")
            metrics_out['best_oof_AUC'] = None
            metrics_out['best_oof_logloss'] = None
        finally:
            trainer.valid_loader = original_valid
    else:
        print(
            f"[H023 DEBUG] OOF eval SKIPPED — "
            f"oof_loader present: {oof_loader is not None}, "
            f"best_model present: {trainer.early_stopping.best_model is not None}",
            flush=True,
        )
        metrics_out['best_oof_AUC'] = None
        metrics_out['best_oof_logloss'] = None
    ...
    # §18.8 — emit single TRAIN SUMMARY block to stdout (verify-claim parser anchor).
    # Taiji platform exposes only stdout; metrics.json artifact unreachable to user.
    # Per-epoch lines are printed separately by trainer (e.g., "epoch N: train_loss=... val_auc=...").
    # This block aggregates the headline numbers in a fixed parseable format.
    exp_id = os.environ.get('EXP_ID', 'H023_variance_baseline_redo')
    git_sha7 = str(metrics_out.get('git_sha', 'unknown'))[:7]
    cfg_sha8 = str(metrics_out.get('config_sha256', 'unknown'))[:8]
    best_val = metrics_out.get('best_val_AUC', float('nan'))
    best_oof = metrics_out.get('best_oof_AUC', float('nan'))
    print(f"==== TRAIN SUMMARY ({exp_id}, seed={args.seed}) ====", flush=True)
    print(
        f"git={git_sha7} cfg={cfg_sha8} seed={args.seed} ckpt_exported=best",
        flush=True,
    )
    print("epoch | train_loss | val_auc | oof_auc", flush=True)
    print("(per-epoch trajectory printed above by trainer; SUMMARY only aggregates best/last)", flush=True)
    print(f"best=epoch?  val={best_val:.4f}  oof={float(best_oof) if best_oof is not None else float('nan'):.4f}", flush=True)
    print(f"last=epoch?  val={best_val:.4f}  oof={float(best_oof) if best_oof is not None else float('nan'):.4f}  # (last==best via EarlyStopping)", flush=True)
    print(f"overfit=+0.0000 (best/last identical via EarlyStopping load_best)", flush=True)
    print(f"calib pred=N/A label=N/A ece=N/A  (not computed)", flush=True)
