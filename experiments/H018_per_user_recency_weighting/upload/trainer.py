...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # clip [clip_min, clip_max] for variance safety.
        ...
        # H018 — Per-user exp time-decay loss weighting state
        ...
        if use_per_user_recency:
            logging.info(
                ...
                f"(tau={recency_tau_days:.1f}d, clip=[{recency_weight_clip_min:.2f}, "
                f"{recency_weight_clip_max:.2f}], per-batch normalize mean=1.0)"
            )
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # weight = exp(-days_since_last_event / tau), per-batch normalize
        # (mean=1.0 → loss scale preserved → lr/optim 영향 0), then clip
        # [clip_min, clip_max] for variance safety. Users with no history
        # get gap → large → exp ≈ 0 → clip_min rescue.
