...
def parse_args() -> argparse.Namespace:
    ...
    # days_since_last_event). Mechanism stack (NS xattn + DCN-V2) byte-identical.
    parser.add_argument('--use_per_user_recency', action='store_true', default=False,
                        help='H018: enable per-user exp time-decay loss weighting. '
                             'weight = exp(-days_since_last_event / tau), per-batch '
                             'normalize mean=1.0, clip [clip_min, clip_max]. '
                             'days_since_last_event derived from max(timestamp − max_seq_ts) '
                             'across 4 domain seqs (a/b/c/d) per row.')
    parser.add_argument('--recency_tau_days', type=float, default=14.0,
                        help='H018: exp decay tau in days. Default 14 (1 week active, '
                             '2 weeks recent, 1 month stale).')
    parser.add_argument('--recency_weight_clip_min', type=float, default=0.1,
                        help='H018: weight clip lower bound (variance safety, default 0.1).')
    parser.add_argument('--recency_weight_clip_max', type=float, default=3.0,
                        help='H018: weight clip upper bound (variance safety, default 3.0).')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        # H018 — Per-user exp time-decay recency weighting (replaces H015 per-batch linear).
    ...
    # Per-epoch lines printed separately by trainer above; SUMMARY aggregates best/last.
    exp_id = os.environ.get('EXP_ID', 'H018_per_user_recency_weighting')
    ...
    print(f"last=epoch?  val={best_val:.4f}  oof={float(best_oof) if best_oof is not None else float('nan'):.4f}  # (last==best via EarlyStopping load_best)", flush=True)
    print(f"overfit=+0.0000 (best/last identical via EarlyStopping)", flush=True)
