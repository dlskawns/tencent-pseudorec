...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # zero_copy_only=False + fill_null(0): inference data 의 null label_time 안전 처리 (test eval 시 label_time null 가능).
        ...
        result = {
            ...
            'label_time': torch.from_numpy(label_times),  # H015 carry-forward (kept for back-compat; H018 uses days_since_last_event below)
            'user_id': user_ids,
            '_seq_domains': self.seq_domains,
        }

        # H018 — per-user recency: accumulate max(ts) across all 4 domain seqs
        # for days_since_last_event derivation. Initialized to 0 (== "no history",
        # which yields gap = current timestamp = very large → weight ≈ 0 after
        # exp decay, but per-batch normalize + clip prevents underflow).
        ...
        for domain in self.seq_domains:
            ...
            if ts_ci is not None:
                ...
                # H018 — accumulate per-row max ts across all domains.
                # Use ts_padded.max(axis=1) only over valid positions (padded
                # positions are 0, so max ignores them unless all are 0).
                domain_max_ts = ts_padded.max(axis=1)
                last_event_ts = np.maximum(last_event_ts, domain_max_ts)
        ...
        # H018 — per-user days_since_last_event for exp decay loss weighting.
        # gap (sec) = max(timestamp − max_event_ts_across_4_domains, 0).
        # Convert to days. trainer applies exp(-days/tau) → per-batch normalize
        # → clip [recency_weight_clip_min, recency_weight_clip_max].
        # Users with no history (last_event_ts == 0) get gap = timestamp (large)
        # → exp ≈ 0 → clip rescue.
        gap_seconds = np.maximum(timestamps - last_event_ts, 0).astype(np.float32)
        days_since_last_event = gap_seconds / 86400.0  # (B,) float32
