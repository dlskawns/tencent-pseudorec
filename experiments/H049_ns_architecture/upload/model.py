...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H049 — NS architecture refinement (slot-type embedding for 4 NS segments)
        use_ns_slot_type_emb: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H049 — NS slot type embedding (4 types: user_ns, user_dense, item_ns, item_dense)
        self.use_ns_slot_type_emb = use_ns_slot_type_emb
        if use_ns_slot_type_emb:
            self.ns_type_emb = nn.Embedding(4, d_model)
            nn.init.normal_(self.ns_type_emb.weight, std=0.02)
            type_ids = []
            type_ids.extend([0] * num_user_ns)
            if self.has_user_dense:
                type_ids.append(1)
            type_ids.extend([2] * num_item_ns)
            if self.has_item_dense:
                type_ids.append(3)
            self.register_buffer('ns_type_ids', torch.tensor(type_ids, dtype=torch.long))
            logging.info(
                f"H049 NS slot type emb enabled: 4 types over {self.num_ns} NS slots "
                f"(user_ns={num_user_ns}, dense+1, item_ns={num_item_ns}, dense+1)"
            )
    ...
    def _build_token_streams(
        ...
        # H049 — slot-type embedding broadcast over batch dim
        if self.use_ns_slot_type_emb:
            type_emb = self.ns_type_emb(self.ns_type_ids)  # (num_ns, D)
            ns_tokens = ns_tokens + type_emb.unsqueeze(0)  # (1, num_ns, D) → (B, num_ns, D)
