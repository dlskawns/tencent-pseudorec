...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H061 — TWIN candidate Q enrichment with time signal (upstream of GSU+ESU)
        use_twin_candidate_time: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H061 — TWIN candidate Q enrichment with time signal
        self.use_twin_candidate_time = use_twin_candidate_time
        if use_twin_candidate_time:
            self.candidate_time_proj = nn.Linear(1, d_model)
            nn.init.normal_(self.candidate_time_proj.weight, std=0.02)
            nn.init.zeros_(self.candidate_time_proj.bias)
            logging.info(
                f"H061 TWIN candidate time enrichment enabled: "
                f"candidate += 0.1 * Linear(log1p(max_time_bucket))"
            )
    ...
    def _compute_twin_residual(
        ...
        # H061: enrich candidate with time signal (max time bucket across domains).
        if self.use_twin_candidate_time:
            time_per_domain = []
            for domain in self.seq_domains:
                tb = inputs.seq_time_buckets[domain]  # (B, L) int
                time_per_domain.append(tb.max(dim=1).values)  # (B,)
            max_time = torch.stack(time_per_domain, dim=1).max(dim=1).values.float()  # (B,)
            time_log = torch.log1p(max_time + 1.0).unsqueeze(-1)  # (B, 1)
            time_emb = self.candidate_time_proj(time_log)  # (B, D)
