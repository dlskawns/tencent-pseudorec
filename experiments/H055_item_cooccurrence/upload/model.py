...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H053 — Cross-domain pool contrastive (item-side semantic via cross-domain alignment)
        use_cross_domain_contrast: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H053 — Cross-domain pool contrastive (no params, just forward tuple flag)
        self.use_cross_domain_contrast = use_cross_domain_contrast
        if use_cross_domain_contrast:
            logging.info(
                f"H053 cross-domain pool contrastive enabled: forward returns (logits, per_domain_pools)"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        H053: returns (logits, per_domain_pools (B, 4, D)) tuple when
        use_cross_domain_contrast=True. Otherwise returns logits only (H019 byte-identical).
        """
        ...
        # H053 — return per-domain pooled seq tokens for cross-domain contrastive in trainer.
        # 4 도메인 같은 user 의 pool 은 같아야 (positive), 다른 user → negative.
        if self.use_cross_domain_contrast:
            per_domain_pools = []
            for tokens, mask in zip(seq_tokens_list, seq_masks_list):
                valid = (~mask).float().unsqueeze(-1)
                sum_valid = valid.sum(dim=1).clamp(min=1.0)
                pooled = (tokens * valid).sum(dim=1) / sum_valid  # (B, D)
                per_domain_pools.append(pooled)
            stacked = torch.stack(per_domain_pools, dim=1)  # (B, 4, D)
