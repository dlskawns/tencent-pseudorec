...
def parse_args() -> argparse.Namespace:
    ...
    # H053 — Cross-domain pool contrastive
    parser.add_argument('--use_cross_domain_contrast', action='store_true', default=False,
                        help='H053: enable cross-domain pool InfoNCE on per-domain seq pools. '
                             'Same user\'s 2 random domain pools = positive, others = negative.')
    parser.add_argument('--cross_domain_contrast_lambda', type=float, default=0.1,
                        help='H053: cross-domain contrastive auxiliary loss weight.')
    parser.add_argument('--cross_domain_contrast_temperature', type=float, default=0.1,
                        help='H053: InfoNCE temperature.')
...
def main() -> None:
    ...
    model_args = {
        ...
        # H053 — Cross-domain pool contrastive
        "use_cross_domain_contrast": args.use_cross_domain_contrast,
    }
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        cross_domain_contrast_lambda=args.cross_domain_contrast_lambda,    # H053
        cross_domain_contrast_temperature=args.cross_domain_contrast_temperature,    # H053
