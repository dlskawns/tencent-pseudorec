...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        cross_domain_contrast_lambda: float = 0.0,    # H053
        cross_domain_contrast_temperature: float = 0.1,    # H053
        ...
        self.cross_domain_contrast_lambda: float = cross_domain_contrast_lambda    # H053
        self.cross_domain_contrast_temperature: float = cross_domain_contrast_temperature    # H053
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H053: model returns (logits, per_domain_pools (B, 4, D)) tuple.
        if isinstance(model_out, tuple):
            raw_logits, per_domain_pools = model_out
        else:
            raw_logits = model_out
            per_domain_pools = None
        logits = raw_logits.squeeze(-1)  # (B,)
        ...
        # H053 — Cross-domain pool InfoNCE: same user's 2 random domain pools = positive,
        # other users' pools = negative. Forces user representation cross-domain consistency.
        # Direct attack on §3.5 4 domains jaccard 0.7~10% overlap signal.
        if per_domain_pools is not None and self.cross_domain_contrast_lambda > 0:
            B, num_d, D = per_domain_pools.shape
            # Pick 2 random domains per training step.
            domain_idx = torch.randperm(num_d, device=per_domain_pools.device)[:2]
            ...
            sim = (anchor_n @ positive_n.T) / self.cross_domain_contrast_temperature  # (B, B)
            targets = torch.arange(B, device=sim.device)
            cd_loss = F.cross_entropy(sim, targets)
            loss = loss + self.cross_domain_contrast_lambda * cd_loss
