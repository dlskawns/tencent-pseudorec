...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    # H011: rebuild aligned_user_dense_specs from cfg + schemas (mirrors train.py).
    aligned_user_dense_specs = None
    if cfg.get("use_aligned_pair_encoding", False):
        aligned_pair_fids = cfg.get("aligned_pair_fids", [62, 63, 64, 65, 66, 89, 90, 91])
        user_int_fid_to_idx = {
            fid: i for i, (fid, _, _) in enumerate(eval_ds.user_int_schema.entries)
        }
        user_dense_fid_to_spec = {
            fid: (offset, length) for fid, offset, length in eval_ds.user_dense_schema.entries
        }
        aligned_user_dense_specs = {}
        for fid in aligned_pair_fids:
    ...
    model = PCVRHyFormer(
        ...
        # H011 — aligned <id, weight> pair encoding
        use_aligned_pair_encoding=cfg.get("use_aligned_pair_encoding", False),
