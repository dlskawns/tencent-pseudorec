...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_aligned_pair_encoding', action='store_true', default=False,
                        help='H011: enable aligned <id, weight> pair encoding at input '
                             'embedding lookup. For each aligned fid, replace baseline '
                             'mean-pool with per-row L1-normalized weighted mean using '
                             'user_dense_feats slice as weights (Option α, parameter-free).')
    parser.add_argument('--aligned_pair_fids', type=int, nargs='+',
                        default=[62, 63, 64, 65, 66, 89, 90, 91],
                        help='H011: list of fid numbers that are verified aligned <id, '
                             'weight> pairs (both in user_int_feats and user_dense_feats). '
                             'Default = 8 fids verified in eda/out/aligned_audit.json.')
...
def main() -> None:
    ...
    # H011: build aligned_user_dense_specs = {fid_idx_in_user_int: (dense_offset, dense_dim)}.
    aligned_user_dense_specs = None
    if args.use_aligned_pair_encoding:
        user_int_fid_to_idx = {
            fid: i for i, (fid, _, _) in enumerate(pcvr_dataset.user_int_schema.entries)
        }
        user_dense_fid_to_spec = {
            fid: (offset, length) for fid, offset, length in pcvr_dataset.user_dense_schema.entries
        }
        aligned_user_dense_specs = {}
        for fid in args.aligned_pair_fids:
            if fid in user_int_fid_to_idx and fid in user_dense_fid_to_spec:
                aligned_user_dense_specs[user_int_fid_to_idx[fid]] = user_dense_fid_to_spec[fid]
            else:
                logging.warning(
                    f"H011: fid {fid} requested as aligned but missing in "
                    f"{'user_int' if fid not in user_int_fid_to_idx else 'user_dense'} "
                    f"schema — skipped."
                )
        logging.info(
            f"H011 aligned_pair_encoding ENABLED: {len(aligned_user_dense_specs)} fids "
            f"(fid_idx → (dense_offset, dense_dim)): {aligned_user_dense_specs}"
        )
    ...
    model_args = {
        ...
        "use_aligned_pair_encoding": args.use_aligned_pair_encoding,
        "aligned_user_dense_specs": aligned_user_dense_specs,
    }
