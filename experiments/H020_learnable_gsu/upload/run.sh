#!/bin/bash
# Platform run.sh for H020_learnable_gsu.
# Sub-H of H019 (TWIN paradigm shift first entry, cloud measurable PASS
# 0.839674). Single mutation vs H019: --twin_learnable_gsu flag enables
# learnable GSU projection (W_q, W_k: Linear(d_model, d_model//4, bias=False))
# before inner product. Paper-faithful TWIN GSU form (Tencent 2024 RecSys).
#
...
export EXP_ID="H020_learnable_gsu"
...
echo "[run.sh] mode=H020 TWIN learnable GSU (sub-H of H019, scoring axis)"
...
echo "[run.sh] training complete (H020); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'H020 TWIN retrieval enabled' log line with learnable_gsu=True"
