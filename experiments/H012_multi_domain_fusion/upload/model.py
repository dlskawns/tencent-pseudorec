...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def __init__(
        ...
        # H012 — Multi-domain MoE on NS-tokens (MMoE single-layer)
        use_multi_domain_moe: bool = False,
        num_experts: int = 4,
        moe_ffn_hidden: int = 128,
    ) -> None:
        super().__init__()
        self.num_sequences = num_sequences
        self.num_queries = num_queries
        ...
        self.use_multi_domain_moe = use_multi_domain_moe
        ...
        # H012 — Multi-domain MoE on NS-tokens. Applied after NS xattn,
        # before query decoder. NS dimension preserved → DCN-V2 input unchanged.
        if use_multi_domain_moe:
            self.moe_block = MultiDomainMoEBlock(
                d_model=d_model,
                num_experts=num_experts,
                ffn_hidden=moe_ffn_hidden,
                dropout=dropout,
                log_gate_entropy=log_attn_entropy,
            )
        else:
            self.moe_block = None
    ...
    def forward(
        ...
        # H012 — Multi-domain MoE on NS-tokens (post NS xattn, pre query decoder).
        # NS dimension preserved → DCN-V2 fusion input token stack unchanged.
        if self.use_multi_domain_moe and self.moe_block is not None:
            ns_tokens = self.moe_block(ns_tokens)
...
# ═══════════════════════════════════════════════════════════════════════════════
# H012 — MultiDomainMoEBlock (MMoE single-layer)
# ═══════════════════════════════════════════════════════════════════════════════


class MultiDomainMoEBlock(nn.Module):
    """H012: Multi-domain Mixture-of-Experts on NS-tokens.

    N expert FFN towers + softmax gate per NS-token. Specialization emerges
    from learning — domains with disjoint vocab (Jaccard ≤ 0.10 in TAAC 2026)
    naturally route to different experts. Residual + LayerNorm preserved.

    Source: MMoE (Ma et al. KDD 2018, arXiv:1810.10739).

    Sample-scale risk (CLAUDE.md §10.9): gate softmax can collapse to 1-2
    experts. Output gate entropy logged when log_gate_entropy=True; threshold
    0.5 × log(num_experts) for collapse warning.
    """
    ...
    def __init__(
        ...
        ffn_hidden: int = 128,
        dropout: float = 0.0,
        log_gate_entropy: bool = False,
    ) -> None:
        super().__init__()
        self.num_experts = num_experts
        self.log_gate_entropy = log_gate_entropy
        self._last_gate_entropy: Optional[torch.Tensor] = None

        # N expert FFNs (Linear → SiLU → Linear), each d_model → d_model.
        ...
        # Gate: per-NS-token → per-expert routing weight.
        self.gate = nn.Linear(d_model, num_experts)
        # Pre-LN on input (mirror RankMixer/DCN-V2 anchor convention).
        self.input_ln = nn.LayerNorm(d_model)

    def forward(self, ns_tokens: torch.Tensor) -> torch.Tensor:
        """ns_tokens: (B, N_NS, D) → (B, N_NS, D), shape preserved.

        residual: out = ns_tokens + Σ gate_i × expert_i(LN(ns_tokens)).
        """
        x = self.input_ln(ns_tokens)                                    # (B, N_NS, D)
        gate_logits = self.gate(x)                                      # (B, N_NS, E)
        gate_w = F.softmax(gate_logits, dim=-1)                         # (B, N_NS, E)

        # Stack expert outputs: (B, N_NS, E, D).
        expert_out = torch.stack([e(x) for e in self.experts], dim=-2)
        # Weighted sum across experts.
        weighted = (expert_out * gate_w.unsqueeze(-1)).sum(dim=-2)      # (B, N_NS, D)

        if self.log_gate_entropy:
            # Per-batch averaged routing distribution → entropy across experts.
            avg_gate = gate_w.mean(dim=(0, 1))                          # (E,)
            avg_gate = avg_gate.clamp(min=1e-8)
            ent = -(avg_gate * avg_gate.log()).sum()
            self._last_gate_entropy = ent.detach()

        return ns_tokens + weighted
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        self.use_multi_domain_moe = use_multi_domain_moe
        self.num_experts = num_experts
        self.moe_ffn_hidden = moe_ffn_hidden
        ...
        if backbone == 'hyformer':
            ...
            self.blocks = nn.ModuleList([
                MultiSeqHyFormerBlock(
                    ...
                    # H012 — Multi-domain MoE on NS-tokens
                    use_multi_domain_moe=use_multi_domain_moe,
                    num_experts=num_experts,
    ...
    def collect_moe_gate_entropies(self) -> Optional[List[Optional[float]]]:
        """H012: Returns per-block MoE gate routing entropy across experts.

        Threshold (CLAUDE.md §10.9 collapse abort): 0.5 × log(num_experts).
        For 4 experts → 0.5 × log(4) ≈ 0.69. Below threshold = collapse.
        Above log(num_experts) ≈ 1.39 = uniform routing (no specialization).
        """
        if self.backbone != 'hyformer' or not self.use_multi_domain_moe:
            return None
        return [
            b.moe_block._last_gate_entropy.item()
            if (b.use_multi_domain_moe and b.moe_block is not None
                and b.moe_block._last_gate_entropy is not None)
            else None
            for b in self.blocks
        ]
