...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_multi_domain_moe', action='store_true', default=False,
                        help='H012: enable Multi-domain MoE on NS-tokens. '
                             'N expert FFNs + softmax gate per NS-token. '
                             'Applied post NS xattn, pre query decoder. '
                             'NS dimension preserved → DCN-V2 fusion input unchanged.')
    parser.add_argument('--num_experts', type=int, default=4,
                        help='H012: number of MoE experts (default 4 = num domains a/b/c/d). '
                             'Effective only with --use_multi_domain_moe.')
    parser.add_argument('--moe_ffn_hidden', type=int, default=128,
                        help='H012: MoE expert FFN hidden dim (default 128 = 2 × d_model). '
                             'Effective only with --use_multi_domain_moe. '
                             '~33K params total at default (4 experts × FFN 64→128→64).')
...
def main() -> None:
    ...
    model_args = {
        ...
        "use_multi_domain_moe": args.use_multi_domain_moe,
        "num_experts": args.num_experts,
        "moe_ffn_hidden": args.moe_ffn_hidden,
    }
    ...
    # ---- H012 — MoE gate entropy diagnostic ----
    # MoE gate softmax routing collapse risk (CLAUDE.md §10.9): threshold
    # = 0.5 × log(num_experts) (collapse) and log(num_experts) (uniform).
    if args.use_multi_domain_moe and args.log_attn_entropy:
        try:
            import math
            if trainer.early_stopping.best_model is not None:
                model.load_state_dict(trainer.early_stopping.best_model)
            ...
            with torch.no_grad():
                trainer._evaluate_step(diag_batch)
            moe_entropies = model.collect_moe_gate_entropies() or []
            metrics_out['moe_gate_entropy_per_block'] = moe_entropies
            log_E = math.log(args.num_experts)
            collapse_threshold = 0.5 * log_E
            metrics_out['moe_collapse_threshold'] = collapse_threshold
            metrics_out['moe_uniform_threshold'] = log_E
            metrics_out['moe_collapse_violation'] = any(
                (e is not None and e < collapse_threshold) for e in moe_entropies
            )
            logging.info(f"moe_gate_entropy_per_block={moe_entropies}, "
                         f"collapse_threshold={collapse_threshold:.4f}, "
                         f"uniform_threshold={log_E:.4f}, "
                         f"collapse_violation={metrics_out['moe_collapse_violation']}")
        except Exception as e:
            logging.warning(f"MoE gate entropy diagnostic failed: {e}")
