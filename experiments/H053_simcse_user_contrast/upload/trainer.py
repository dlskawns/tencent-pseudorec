...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        simcse_lambda: float = 0.0,    # H050 — 0.0 → off (H019 byte-identical)
        simcse_temperature: float = 0.1,    # H050 — InfoNCE temperature
        simcse_dropout: float = 0.1,    # H050 — dropout rate for two views
        ...
        self.simcse_lambda: float = simcse_lambda    # H050
        self.simcse_temperature: float = simcse_temperature    # H050
        self.simcse_dropout: float = simcse_dropout    # H050
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H050: model returns (logits, user_repr) tuple when use_user_simcse.
        if isinstance(model_out, tuple):
            raw_logits, user_repr = model_out
        else:
        ...
        # H050 — SimCSE: 2 dropout views of user_repr, InfoNCE positive=(z1_i,z2_i),
        # negatives=(z1_i,z2_j≠i). Forces user representation invariance to dropout
        # noise = SSL-style self-supervision (Gao et al. 2021 SimCSE paper, recsys 적용).
        if user_repr is not None and self.simcse_lambda > 0:
            z1 = F.dropout(user_repr, p=self.simcse_dropout, training=True)
            z2 = F.dropout(user_repr, p=self.simcse_dropout, training=True)
            z1n = F.normalize(z1, dim=-1)
            z2n = F.normalize(z2, dim=-1)
            sim = (z1n @ z2n.T) / self.simcse_temperature  # (B, B)
            targets = torch.arange(sim.shape[0], device=sim.device)
            simcse_loss = F.cross_entropy(sim, targets)
            loss = loss + self.simcse_lambda * simcse_loss
