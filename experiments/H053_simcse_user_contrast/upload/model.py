...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H050 — SimCSE-style user self-contrastive (SSL via dropout views, paradigm shift)
        use_user_simcse: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H050 — SimCSE-style user self-contrastive (no params, just forward tuple flag)
        self.use_user_simcse = use_user_simcse
        if use_user_simcse:
            logging.info(
                f"H050 SimCSE user self-contrastive enabled: forward returns (logits, output)"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        H050: returns (logits, user_repr=output) tuple when use_user_simcse=True.
        ...
        # H050 — return user representation (backbone output) for SimCSE in trainer.
        if self.use_user_simcse:
