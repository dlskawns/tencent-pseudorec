#!/bin/bash
# Platform run.sh for H019_twin_long_seq_retrieval.
# Single mutation vs H023 baseline: --use_twin_retrieval flag enables
# per-domain TWINBlock (GSU+ESU). Paradigm shift — sequential axis 의
# retrieval form (long-seq capacity → top-K candidate-relevant 선택).
#
...
export EXP_ID="H019_twin_long_seq_retrieval"
...
echo "[run.sh] mode=H019 TWIN per-domain GSU+ESU retrieval (paradigm shift)"
...
echo "[run.sh] training complete (H019); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + TWIN retrieval enabled log line"
