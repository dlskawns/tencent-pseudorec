...
class TWINBlock(nn.Module):
    ...
    H062: optional learnable GSU scoring (low-rank W_q/W_k projection) instead of
    parameter-free inner product. TWIN-internal mutation, single residual preserved.
    """

    def __init__(self, d_model: int, num_heads: int = 4, top_k: int = 64,
                 use_learnable_gsu: bool = False, gsu_rank: int = 16) -> None:
        super().__init__()
        self.d_model = d_model
        self.top_k = top_k
        self.esu = nn.MultiheadAttention(d_model, num_heads, batch_first=True)
        self.norm = nn.LayerNorm(d_model)
        # H062 — learnable GSU scoring (low-rank W_q/W_k for sample budget).
        self.use_learnable_gsu = use_learnable_gsu
        if use_learnable_gsu:
            self.gsu_W_q = nn.Linear(d_model, gsu_rank, bias=False)
            self.gsu_W_k = nn.Linear(d_model, gsu_rank, bias=False)
            nn.init.xavier_uniform_(self.gsu_W_q.weight)
            nn.init.xavier_uniform_(self.gsu_W_k.weight)
    ...
    def forward(
        ...
        B, L, D = history.shape
        K = min(self.top_k, L)
        # GSU — H062: optional learnable W_q/W_k scoring.
        if self.use_learnable_gsu:
            h_proj = self.gsu_W_k(history)               # (B, L, gsu_rank)
            c_proj = self.gsu_W_q(candidate)             # (B, gsu_rank)
            scores = (h_proj * c_proj.unsqueeze(1)).sum(-1)  # (B, L)
        else:
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H062 — TWIN learnable GSU (parameter-free → low-rank W_q/W_k)
        use_twin_learnable_gsu: bool = False,
        twin_gsu_rank: int = 16,
    ) -> None:
        super().__init__()
        ...
        if use_twin_retrieval:
            self.twin_blocks = nn.ModuleDict({
                f'twin_{domain}': TWINBlock(
                    ...
                    top_k=twin_top_k,
                    use_learnable_gsu=use_twin_learnable_gsu,  # H062
                    gsu_rank=twin_gsu_rank,                   # H062
                )
