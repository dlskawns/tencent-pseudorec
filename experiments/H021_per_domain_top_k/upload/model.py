...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H021 — twin_top_k accepts int (uniform) OR dict {seq_a, seq_b, seq_c, seq_d → int} (per-domain)
        ...
        # H021 — top_k policy uniform → per-domain dict
        self.use_twin_retrieval = use_twin_retrieval
        if use_twin_retrieval:
            # H021: accept int (uniform, H019 backward compat) OR dict (per-domain).
