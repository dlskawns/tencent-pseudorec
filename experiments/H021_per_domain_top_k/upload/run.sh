#!/bin/bash
# Platform run.sh for H021_per_domain_top_k.
# Sub-H of H019 (TWIN paradigm shift first entry, cloud measurable PASS
# 0.839674). Single mutation vs H019: --twin_top_k_per_domain "64,64,64,96"
# enables per-domain top_k policy. domain seq_d (p90=2215, longest) 만
# K=64→96 으로 50% 확장. 다른 도메인은 K=64 유지. trainable params 추가 0.
# TWINBlock class byte-identical to H019 — PCVRHyFormer wiring 변경만.
#
...
export EXP_ID="H021_per_domain_top_k"
...
echo "[run.sh] mode=H021 TWIN per-domain top_k (sub-H of H019, quantity axis)"
...
python3 -u "${SCRIPT_DIR}/train.py" \
    ...
    --use_twin_retrieval \
    --twin_top_k 64 \
    --twin_top_k_per_domain "64,64,64,96" \
    --twin_num_heads 4 \
    --twin_gate_init -2.0 \
    --log_attn_entropy \
    "$@"

echo "[run.sh] training complete (H021); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'H021 TWIN per-domain K' log line"
