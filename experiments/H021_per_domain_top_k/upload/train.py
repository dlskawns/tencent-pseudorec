...
def _parse_twin_top_k_arg(args) -> object:
    """H021 — parse --twin_top_k_per_domain string into dict, or fall back
    ...
    logging.info(f"H021 enabled: per-domain K = {parsed}")
    return parsed
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--twin_top_k_per_domain', type=str, default=None,
                        help='H021: comma-separated 4-int (seq_a,seq_b,seq_c,seq_d) '
                             'per-domain K, e.g. "64,64,64,96". '
                             'If set, overrides --twin_top_k. Default None = '
                             'H019 uniform --twin_top_k value.')
