...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--loss_type', type=str, default='bce',
                        choices=['bce', 'focal', 'aux_timestamp'],
                        help='Loss type: bce = BCEWithLogits, focal = Focal Loss, '
                             'aux_timestamp = H038 BCE main + MSE aux on standardized '
                             'log1p(timestamp) regression (action_num=2).')
    parser.add_argument('--aux_lambda', type=float, default=0.1,
                        help='H038: weight for aux MSE loss (loss = BCE_main + λ·MSE_aux).')
    parser.add_argument('--kld_lambda', type=float, default=0.0,
                        help='H042: weight for KLD prior matching term '
                             '(0.0 → off, H038 byte-identical). '
                             'loss += λ_kld · mean_i KL(Bern(σ(logit_main_i)) || Bern(prior)).')
    parser.add_argument('--kld_target_prior', type=float, default=0.124,
                        help='H042: target Bernoulli prior for KLD (default 0.124 = §3.4 class prior).')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        aux_lambda=args.aux_lambda,    # H038
        kld_lambda=args.kld_lambda,    # H042
        kld_target_prior=args.kld_target_prior,    # H042
