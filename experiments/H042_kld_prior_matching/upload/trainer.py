...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        aux_lambda: float = 0.1,    # H038
        kld_lambda: float = 0.0,    # H042 — 0.0 → off (H038 byte-identical)
        kld_target_prior: float = 0.124,    # H042 — §3.4 class prior
        ...
        self.aux_lambda: float = aux_lambda    # H038
        self.kld_lambda: float = kld_lambda    # H042
        self.kld_target_prior: float = kld_target_prior    # H042
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        raw_logits = self.model(model_input)  # (B, action_num)

        if self.loss_type == 'aux_timestamp':
            # H038: BCE main + MSE aux on standardized log1p(timestamp).
            # raw_logits: (B, 2). col 0 = main_logit (binary), col 1 = aux_pred (regression).
            # H042: optional KLD prior matching on per-sample Bernoulli pred.
            logit_main = raw_logits[:, 0]
            aux_pred = raw_logits[:, 1]
            ts = device_batch['timestamp'].float()
            ts_log = torch.log1p(torch.clamp(ts - ts.min() + 1, min=1e-6))
            aux_target = (ts_log - ts_log.mean()) / (ts_log.std() + 1e-6)
            bce = F.binary_cross_entropy_with_logits(logit_main, label)
            mse = F.mse_loss(aux_pred, aux_target)
            loss = bce + self.aux_lambda * mse
            if self.kld_lambda > 0:
                # H042: KL(Bern(p_i) || Bern(prior)), per-sample mean.
                # Pulls overconfident predictions toward class prior — Pereyra
                # et al. 2017 confidence penalty form on Bernoulli output.
                prob = torch.sigmoid(logit_main).clamp(min=1e-6, max=1.0 - 1e-6)
                prior = float(self.kld_target_prior)
                kld_per_sample = (
    ...
    def _evaluate_step(
        ...
        model_input = self._make_model_input(device_batch)
        raw_logits, _ = self.model.predict(model_input)  # (B, action_num), (B, D)
        if self.loss_type == 'aux_timestamp':
            # H038: main_logit (col 0) is the inference logit; aux_pred ignored at eval.
            logits = raw_logits[:, 0]
        else:
            logits = raw_logits.squeeze(-1)
