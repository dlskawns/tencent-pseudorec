...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--focal_gamma', type=float, default=2.0,
                        help='Focal Loss focusing parameter gamma '
                             '(effective only when --loss_type=focal)')
    # H015 — Recency-aware loss weighting (cohort drift mitigation).
    parser.add_argument('--use_recency_loss_weighting', action='store_true', default=False,
                        help='H015: enable per-batch linear recency-aware loss weighting. '
                             'Per-batch label_time min-max → percentile → linear weight in '
                             '[recency_weight_min, recency_weight_max]. Recent samples '
                             'receive higher loss weight, training closer to platform eval '
                             'distribution. Mitigates cohort drift hard ceiling (Frame B).')
    parser.add_argument('--recency_weight_min', type=float, default=0.5,
                        help='H015: minimum loss weight (oldest sample in batch). Default 0.5.')
    parser.add_argument('--recency_weight_max', type=float, default=1.5,
                        help='H015: maximum loss weight (newest sample in batch). Default 1.5. '
                             'Mean weight = (min + max) / 2 = 1.0 (loss scale 보존).')
    parser.add_argument('--recency_weight_form', type=str, default='linear',
                        choices=['linear', 'exp'],
                        help='H017: weight curve. linear (default) = w_min + range*pct. '
                             'exp = w_min * (w_max/w_min)^pct, auto-normalized to mean=1.0.')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        # H015 — Recency-aware loss weighting
        # H017 — recency_weight_form ('linear' | 'exp')
        use_recency_loss_weighting=args.use_recency_loss_weighting,
        recency_weight_min=args.recency_weight_min,
        recency_weight_max=args.recency_weight_max,
        recency_weight_form=args.recency_weight_form,
    )
