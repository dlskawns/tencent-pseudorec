...
import os  # noqa: E402  (used by HEARTBEAT_EVERY_N_STEPS env)
import glob
import shutil
import logging
...
except ImportError:  # eval-container fallback
    class _NoTqdm:
...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        # H015 — Recency-aware loss weighting (per-batch label_time linear scaling)
        # H017 — recency_weight_form: 'linear' | 'exp' (auto-normalized to mean=1.0)
        use_recency_loss_weighting: bool = False,
        recency_weight_min: float = 0.5,
        recency_weight_max: float = 1.5,
        recency_weight_form: str = 'linear',
    ) -> None:
        ...
        # H015 — Recency-aware loss weighting state
        # H017 — recency_weight_form ('linear' | 'exp', auto-normalized to mean=1.0)
        self.use_recency_loss_weighting: bool = use_recency_loss_weighting
        self.recency_weight_min: float = recency_weight_min
        self.recency_weight_max: float = recency_weight_max
        self.recency_weight_form: str = recency_weight_form
        if recency_weight_form not in ('linear', 'exp'):
            raise ValueError(
                f"recency_weight_form must be 'linear' or 'exp', got {recency_weight_form}")

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}")
        if use_recency_loss_weighting:
            logging.info(f"H015 ENABLED: recency-aware loss weighting "
                         f"[{recency_weight_min:.2f}, {recency_weight_max:.2f}] "
                         f"(per-batch linear by label_time)")
    ...
    def train(self) -> None:
        ...
        """
        print("Start training (PCVRHyFormer)", flush=True)
        logging.info("entering training loop")
        self.model.train()
        total_step = 0
        heartbeat_every = max(1, int(os.environ.get("HEARTBEAT_EVERY_N_STEPS", "50")))

        for epoch in range(1, self.num_epochs + 1):
            logging.info(f"epoch {epoch}/{self.num_epochs} start")
            train_pbar = tqdm(enumerate(self.train_loader), total=len(self.train_loader),
                              desc=f"train ep{epoch}/{self.num_epochs}",
                              dynamic_ncols=True)
            loss_sum = 0.0
            ...
            for step, batch in train_pbar:
                ...
                train_pbar.set_postfix({
                    "loss": f"{loss:.4f}",
                    "avg": f"{avg_loss:.4f}",
                    "ep": f"{epoch}/{self.num_epochs}",
                })
                # stdout heartbeat — tqdm goes to stderr which some platforms
                # (Taiji) don't capture in the main log stream. Without this,
                # the user sees no output between "Start training" and
                # "Epoch N, Average Loss: ...".
                if total_step == 1 or total_step % heartbeat_every == 0:
                    pct = 100.0 * (step + 1) / max(1, len(self.train_loader))
                    logging.info(
                        f"[train ep{epoch}/{self.num_epochs}] step {total_step} "
                        f"({step + 1}/{len(self.train_loader)}, {pct:.1f}%) "
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H015 — recency-aware loss weighting (per-batch linear by label_time).
        # When enabled: per-sample loss reduction='none' + weighted mean.
        if self.use_recency_loss_weighting:
            label_time = device_batch['label_time'].float()
            lt_min = label_time.min()
            lt_max = label_time.max()
            if (lt_max - lt_min) > 1e-6:
                pct = (label_time - lt_min) / (lt_max - lt_min)
            else:
                pct = torch.full_like(label_time, 0.5)
            # H015 linear: weights = w_min + (w_max - w_min) × pct, mean = (w_min + w_max)/2.
            # H017 exp: weights = w_min × (w_max/w_min)^pct, then auto-normalize to mean=1.0.
            wrange = self.recency_weight_max - self.recency_weight_min
            if self.recency_weight_form == 'exp':
                # Geometric scaling: pct=0 → w_min, pct=1 → w_max.
                ratio = self.recency_weight_max / max(self.recency_weight_min, 1e-8)
                weights = self.recency_weight_min * torch.pow(
                    torch.tensor(ratio, device=label_time.device, dtype=label_time.dtype),
                    pct,
                )
                # Auto-normalize to mean=1.0 (loss scale 보존).
                weights = weights / weights.mean().clamp(min=1e-8)
            else:
                weights = self.recency_weight_min + wrange * pct  # (B,) linear, mean = (w_min+w_max)/2
            if self.loss_type == 'focal':
                loss_per = sigmoid_focal_loss(
