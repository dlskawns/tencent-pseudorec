...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H043 — item-side DCN-V2 cross on item_ns tokens (item-side feature interaction)
        use_item_side_cross: bool = False,
        item_side_cross_layers: int = 2,
        item_side_cross_rank: int = 4,
    ) -> None:
        super().__init__()
        ...
        # H043 — item-side DCN-V2 cross on item_ns tokens
        self.use_item_side_cross = use_item_side_cross
        if use_item_side_cross:
            self.item_side_cross = DCNV2CrossBlock(
                d_model=d_model,
                n_total=0,
                num_cross_layers=item_side_cross_layers,
                rank=item_side_cross_rank,
                dropout=dropout_rate,
            )
            logging.info(
                f"H043 item-side DCN-V2 cross enabled: "
                f"layers={item_side_cross_layers} rank={item_side_cross_rank}"
            )
    ...
    def _build_token_streams(
        ...
        # H043: explicit item-side feature interaction on item NS tokens.
        # DCN-V2 polynomial cross (low-rank, Pre-LN) — same form as user/global
        # backbone DCN-V2, applied isolated to item-side before NS concat.
        if self.use_item_side_cross:
            item_ns = self.item_side_cross(item_ns)
