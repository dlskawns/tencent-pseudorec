...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    model = PCVRHyFormer(
        ...
        use_item_side_cross=cfg.get("use_item_side_cross", False),
        item_side_cross_layers=cfg.get("item_side_cross_layers", 2),
        item_side_cross_rank=cfg.get("item_side_cross_rank", 4),
    ).to(device)
