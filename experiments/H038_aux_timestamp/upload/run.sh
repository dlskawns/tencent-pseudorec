#!/bin/bash
# Platform run.sh for H038_aux_timestamp.
# Data-signal-driven (CLAUDE.md §0.5): backbone gradient signal 부족 — main
# task BCE 만 으로 weak supervision. Aux task = MSE on standardized
# log1p(timestamp). Backbone 이 timestamp 를 explicit 으로 encode 하도록
# multi-task. action_num=2 (main + aux), λ=0.1.
...
export EXP_ID="H038_aux_timestamp"
...
echo "[run.sh] mode=H038 aux_timestamp (BCE main + MSE aux on log1p(timestamp), λ=0.1)"
...
echo "[run.sh] training complete (H038); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'loss_type=aux_timestamp action_num=2 aux_lambda=0.1' indicator"
