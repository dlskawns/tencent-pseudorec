...
def parse_args() -> argparse.Namespace:
    ...
    # ---- H007 anchor: candidate-aware cross-attention pooling ----
    parser.add_argument('--use_candidate_summary_token', action='store_true', default=False,
                        help='H007: per-domain candidate-aware cross-attention pooling '
                             '(DIN/CAN/SIM/TWIN/HSTU/OneTrans family idea, modern multi-head '
                             'xattn implementation). Prepends a candidate-attended summary '
                             'token to each sequence so downstream decoder sees a '
                             'candidate-aware context anchor at position 0.')
    parser.add_argument('--candidate_summary_num_heads', type=int, default=0,
                        help='Multi-head count for CandidateSummaryToken; '
                             '0 = inherit --num_heads.')
...
def main() -> None:
    ...
    model_args = {
        ...
        # H007 anchor — candidate-aware cross-attention pooling
        "use_candidate_summary_token": args.use_candidate_summary_token,
        "candidate_summary_num_heads": args.candidate_summary_num_heads,
    }
    ...
    if args.backbone == 'onetrans' and args.log_attn_entropy:
        try:
