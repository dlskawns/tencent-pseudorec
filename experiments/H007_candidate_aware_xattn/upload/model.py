...
# ═══════════════════════════════════════════════════════════════════════════════
# CandidateSummaryToken (H007 — candidate-aware cross-attention pooling)
# Mechanism class: candidate-as-attention-query
#   (DIN/CAN/SIM/TWIN/HSTU/OneTrans family idea, modern multi-head xattn impl)
# ═══════════════════════════════════════════════════════════════════════════════


class CandidateSummaryToken(nn.Module):
    """Per-domain candidate-aware cross-attention pooling.

    Mechanism: candidate item embedding is the attention query; per-domain
    sequence tokens are K and V. Output is one candidate-attended pooled
    summary token, prepended to the sequence so that downstream blocks see
    a candidate-aware "context anchor" at position 0.

    Input
    -----
    seq_tokens      (B, L, D) per-domain sequence representation.
    padding_mask    (B, L) bool, True = padded position.
    candidate_token (B, 1, D) pooled candidate item representation.

    Output
    ------
    (B, 1, D) candidate-attended pooled summary.

    Implementation notes
    --------------------
    - Pre-LN convention (separate LayerNorms for Q and K/V).
    - Standard multi-head cross-attention (no RoPE — candidate has no positional
      meaning relative to sequence positions).
    - Padding mask aware (-inf on padded keys).
    - All-pad-row guard: rows where every key is padded receive zero attention
      output (no NaN propagation).
    """
    ...
    def forward(
        ...
        q = self.q_proj(self.ln_q(candidate_token))           # (B, 1, D)
        kv = self.kv_proj(self.ln_kv(seq_tokens))             # (B, L, 2D)
        ...
        attn = torch.matmul(q, k.transpose(-2, -1)) * self.scale   # (B, H, 1, L)
        if padding_mask is not None:
            mask_expand = padding_mask.unsqueeze(1).unsqueeze(2)   # (B, 1, 1, L)
            attn = attn.masked_fill(mask_expand, float('-inf'))
        ...
        if all_pad is not None:
            zero_mask = all_pad.unsqueeze(1).unsqueeze(2).expand_as(attn_probs)
            attn_probs = torch.where(
                zero_mask, torch.zeros_like(attn_probs), attn_probs,
            )
        attn_probs = self.attn_dropout(attn_probs)

        out = torch.matmul(attn_probs, v)                     # (B, H, 1, head_dim)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_candidate_summary_token: bool = False,
        candidate_summary_num_heads: int = 0,
    ) -> None:
        super().__init__()
        ...
        # H007 — candidate-aware cross-attention pooling
        self.use_candidate_summary_token = use_candidate_summary_token
        # 0 ⇒ inherit num_heads from outer config
        self.candidate_summary_num_heads = (
            candidate_summary_num_heads if candidate_summary_num_heads > 0 else num_heads
        )
        ...
        # H007 — per-domain CandidateSummaryToken modules.
        # Each domain owns an independent cross-attention pool so domain-specific
        # candidate-history patterns can be learned separately.
        if self.use_candidate_summary_token:
            self.candidate_summary_modules = nn.ModuleDict({
                domain: CandidateSummaryToken(
                    d_model=d_model,
                    num_heads=self.candidate_summary_num_heads,
                    dropout=dropout_rate,
                )
                for domain in self.seq_domains
            })
        else:
            self.candidate_summary_modules = None
    ...
    def _build_token_streams(
        ...
        # H007 — candidate-aware summary token prepended per domain.
        # Candidate token = mean pool of (item_ns + item_dense_tok). Each domain
        # owns its own CandidateSummaryToken module (per-domain pattern learned).
        # Result: seq length grows by 1, position 0 = candidate-attended summary.
        if self.use_candidate_summary_token and self.candidate_summary_modules is not None:
            cand_parts = [item_ns]
            if item_dense_tok is not None:
                cand_parts.append(item_dense_tok)
            candidate_token = torch.cat(cand_parts, dim=1).mean(dim=1, keepdim=True)  # (B, 1, D)

            new_seq_tokens: List[torch.Tensor] = []
            new_seq_masks: List[torch.Tensor] = []
            for d_idx, domain in enumerate(self.seq_domains):
                seq_t = seq_tokens_list[d_idx]
                seq_m = seq_masks_list[d_idx]
                summary = self.candidate_summary_modules[domain](
                    seq_t, seq_m, candidate_token,
                )  # (B, 1, D)
                seq_t_new = torch.cat([summary, seq_t], dim=1)        # (B, L+1, D)
                B = seq_m.shape[0]
                pad_prefix = torch.zeros(B, 1, dtype=torch.bool, device=seq_m.device)
                seq_m_new = torch.cat([pad_prefix, seq_m], dim=1)     # (B, L+1)
                new_seq_tokens.append(seq_t_new)
                new_seq_masks.append(seq_m_new)
            seq_tokens_list = new_seq_tokens
            seq_masks_list = new_seq_masks
    ...
    def collect_attn_entropies(self) -> Optional[List[Optional[float]]]:
        """Returns per-layer mean attention entropy from OneTrans backbone (None for HyFormer)."""
