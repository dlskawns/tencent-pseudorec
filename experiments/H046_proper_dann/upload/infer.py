...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    model = PCVRHyFormer(
        ...
        # H046 — proper DANN cohort debias (cohort_head module needed for state_dict load)
        use_dann_cohort=cfg.get("use_dann_cohort", False),
        dann_cohort_lambda=cfg.get("dann_cohort_lambda", 0.1),
    ).to(device)
