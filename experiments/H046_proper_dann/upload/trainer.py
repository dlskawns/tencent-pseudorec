...
class PCVRHyFormerRankingTrainer:
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H046: model returns (logits, cohort_pred) when use_dann_cohort=True.
        ...
        # H046 — proper DANN: cohort MSE loss on standardized log1p(timestamp).
        # GRL inside model handles backbone gradient reversal; cohort_head trains
        # POSITIVELY (good predictor). Backbone forgets cohort signal.
        if cohort_pred is not None:
            ts = device_batch['timestamp'].float()
            ts_log = torch.log1p(torch.clamp(ts - ts.min() + 1, min=1e-6))
            cohort_target = (ts_log - ts_log.mean()) / (ts_log.std() + 1e-6)
            cohort_loss = F.mse_loss(cohort_pred, cohort_target)
            loss = loss + cohort_loss  # GRL scales backbone grad; cohort_head sees full grad
