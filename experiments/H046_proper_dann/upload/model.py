...
class _GradReverse(torch.autograd.Function):
    """H046 — Gradient reversal layer (Ganin & Lempitsky 2015 DANN), proper form.

    Placed BETWEEN backbone output and cohort head:
      forward: x → identity (cohort head trains positively)
      backward: cohort_head grad → ×(−lambd) → backbone (forgets cohort signal)
    Fixes H044 design flaw (where GRL on raw_logits caused both clsfier-aux and
    backbone to receive reversed grad, no actual discriminator → loss diverged).
    """
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H046 — proper DANN cohort debias (separate cohort head + GRL)
        use_dann_cohort: bool = False,
        dann_cohort_lambda: float = 0.1,
    ) -> None:
        super().__init__()
        ...
        # H046 — proper DANN cohort debias (separate cohort regression head)
        self.use_dann_cohort = use_dann_cohort
        self.dann_cohort_lambda = float(dann_cohort_lambda)
        if use_dann_cohort:
            self.cohort_head = nn.Linear(d_model, 1)
            logging.info(
                f"H046 proper DANN cohort head enabled: "
                f"dann_cohort_lambda={dann_cohort_lambda} (GRL between backbone and cohort_head)"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        H046: returns (logits, cohort_pred) tuple when use_dann_cohort=True.
        Otherwise returns logits only (H019 byte-identical).
        """
        ...
        # H046 — proper DANN: cohort_head trained POSITIVELY (good predictor),
        # backbone gradient reversed via GRL → backbone forgets cohort signal.
        if self.use_dann_cohort:
            reversed_out = _GradReverse.apply(output, self.dann_cohort_lambda)
            cohort_pred = self.cohort_head(reversed_out).squeeze(-1)  # (B,)
            return logits, cohort_pred
        return logits
