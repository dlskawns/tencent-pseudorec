...
class TWINBlock(nn.Module):
    ...
    H019: ESU = single MultiheadAttention layer.
    H034: ESU = stack of `esu_num_layers` MultiheadAttention layers with
          residual + LayerNorm between (Pre-LN style). num_layers=1 →
          H019 byte-equivalent behavior.
    """
    ...
    def __init__(
        ...
        num_heads: int = 4,
        top_k: int = 64,
        esu_num_layers: int = 1,
    ) -> None:
        super().__init__()
        self.d_model = d_model
        self.top_k = top_k
        self.esu_num_layers = esu_num_layers
        if esu_num_layers == 1:
            # H019 backward-compat: single MHA + final LN.
            self.esu = nn.MultiheadAttention(d_model, num_heads, batch_first=True)
            self.norm = nn.LayerNorm(d_model)
            self.esu_layers = None
            self.esu_norms = None
        elif esu_num_layers >= 2:
            # H034: stacked MHA with intermediate residual + LN.
            self.esu = None
            self.norm = None
            self.esu_layers = nn.ModuleList([
                nn.MultiheadAttention(d_model, num_heads, batch_first=True)
                for _ in range(esu_num_layers)
            ])
            self.esu_norms = nn.ModuleList([
                nn.LayerNorm(d_model) for _ in range(esu_num_layers)
            ])
        else:
            raise ValueError(f"esu_num_layers must be ≥ 1, got {esu_num_layers}")
    ...
    def forward(
        ...
        if self.esu_num_layers == 1:
            ...
            # H034 path — stacked MHA with residual + LN per layer.
            x = candidate_q
            for layer, ln in zip(self.esu_layers, self.esu_norms):
                attn_out, _ = layer(
                    x, topk_history, topk_history,
                    key_padding_mask=topk_pad_mask,
                )
                x = ln(x + attn_out)
            attended = x.squeeze(1)  # (B, D)
        return attended
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H034 — ESU multi-layer (capacity axis on retrieved tokens)
        twin_esu_num_layers: int = 1,
    ) -> None:
        super().__init__()
        ...
        # H034 — ESU multi-layer flag passed to TWINBlock
        ...
        if use_twin_retrieval:
            self.twin_blocks = nn.ModuleDict({
                f'twin_{domain}': TWINBlock(
                    ...
                    top_k=twin_top_k,
                    esu_num_layers=twin_esu_num_layers,
                )
            ...
            logging.info(
                f"H034 TWIN retrieval enabled: top_k={twin_top_k} "
                f"num_heads={twin_num_heads} gate_init={twin_gate_init} "
                f"esu_num_layers={twin_esu_num_layers}"
            )
        else:
