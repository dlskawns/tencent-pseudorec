#!/bin/bash
# Platform run.sh for H034_esu_2layer.
# Sub-H of H019 (TWIN paradigm shift, cloud measurable PASS 0.839674).
# Single mutation vs H019: --twin_esu_num_layers 2 — ESU 를 1-layer →
# 2-layer MultiheadAttention stack (intermediate residual + LayerNorm).
# 가져온 token 의 처리 capacity 가 retrieval lever 인지 검증 (axis A3 in
# upload_patch.md). H020/H021 (scoring axes) 모두 NOOP 일 때 가장 informative.
#
...
echo "[run.sh] mode=H034 TWIN ESU 2-layer (sub-H of H019, capacity axis)"
...
python3 -u "${SCRIPT_DIR}/train.py" \
    ...
    --twin_num_heads 4 \
    --twin_gate_init -2.0 \
    --twin_esu_num_layers 2 \
    --log_attn_entropy \
    "$@"

echo "[run.sh] training complete (H034); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'H034 TWIN retrieval enabled' log line with esu_num_layers=2"
