...
# ═══════════════════════════════════════════════════════════════════════════════
# H035 — HSTU Encoder (Meta 2024 ICML, arXiv:2402.17152, "Actions Speak Louder
# than Words: Trillion-Parameter Sequential Transducers")
# Paper-faithful core: pointwise sigmoid-attention (no softmax) + gated linear
# unit (silu-gated by U projection).
# ═══════════════════════════════════════════════════════════════════════════════


class HSTUEncoder(nn.Module):
    """HSTU layer (Meta 2024) — paper-inspired sequential transduction.

    Replaces softmax-attention with pointwise silu-attention + per-token gating.
    Matches TransformerEncoder forward signature for drop-in swap.

    Mechanism (per-paper, eq. 1-3):
        h = SiLU(LN(x) · W_uvqk).chunk(4) → U, V, Q, K
        A = silu(QK^T / sqrt(head_dim)) / L          # length-normalized
        Y = A @ V
        out = LN(W_o · (Y ⊙ U)) + x                  # gated residual
    """
    ...
    def __init__(
        ...
        d_model: int,
        num_heads: int,
        hidden_mult: int = 1,        # H035: paper uses 1× (no FFN expansion). Arg present for signature compat.
        dropout: float = 0.0,
    ) -> None:
        super().__init__()
        ...
        self.num_heads = num_heads
        self.head_dim = d_model // num_heads
        # Single big projection for U, V, Q, K (paper: combined linear).
        self.norm_in = nn.LayerNorm(d_model)
        self.uvqk = nn.Linear(d_model, 4 * d_model, bias=False)
    ...
    def forward(
        ...
        Args:
            ...
            rope_cos / rope_sin: ignored — HSTU uses pointwise attention,
                positional info absorbed by attention structure (no RoPE
                needed). Args present for TransformerEncoder signature compat.
        Returns:
            (out: (B, L, D), key_padding_mask: same as input).
        """
        B, L, D = x.shape
        # Pre-norm + SiLU + project to 4D — paper eq. 1.
        h = F.silu(self.uvqk(self.norm_in(x)))  # (B, L, 4D)
        u, v, q, k = h.chunk(4, dim=-1)         # each (B, L, D)
        # Multi-head reshape.
        q = q.view(B, L, self.num_heads, self.head_dim).transpose(1, 2)  # (B, H, L, hd)
        k = k.view(B, L, self.num_heads, self.head_dim).transpose(1, 2)
        v = v.view(B, L, self.num_heads, self.head_dim).transpose(1, 2)
        # Pointwise attention: silu(QK^T / sqrt(d)) / L — paper eq. 2.
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)  # (B, H, L, L)
        if key_padding_mask is not None:
            # (B, L) → (B, 1, 1, L). Mask padded keys with 0 (silu(0)=0 → no contribution).
            mask_exp = key_padding_mask.unsqueeze(1).unsqueeze(1)
            scores = scores.masked_fill(mask_exp, 0.0)
        attn = F.silu(scores) / max(L, 1)        # length-normalize.
        attn = self.dropout(attn)
        # Apply attention.
        out = torch.matmul(attn, v)              # (B, H, L, hd)
        out = out.transpose(1, 2).contiguous().view(B, L, D)
        # Gate with U + output projection + residual — paper eq. 3.
        out = self.proj_out(out * u)
        out = self.norm_out(out) + x
...
def create_sequence_encoder(
    ...
    Args:
        encoder_type: One of 'swiglu', 'transformer', 'longer', or 'hstu'.
        d_model: Model dimension.
        num_heads: Number of attention heads (used by transformer/longer/hstu).
        hidden_mult: FFN expansion multiplier (ignored by hstu — uses 1×).
    ...
    if encoder_type == 'swiglu':
        ...
        # H035: HSTU paper-inspired pointwise silu-attention + gated linear unit.
...
# ═══════════════════════════════════════════════════════════════════════════════
# H019 — TWINBlock (per-domain TWIN GSU+ESU long-seq retrieval, paradigm shift)
# ═══════════════════════════════════════════════════════════════════════════════
# Motivation: §3.5 seq p90=1393~2215 vs trunc 64-128 → 95%+ tail loss.
# 11 H ceiling 0.832~0.836 mechanism mutation 무관 → paradigm shift.
#
# Mechanism (TWIN, Tencent 2024 RecSys arXiv:2302.02352):
#   GSU: parameter-free inner-product score per history token vs candidate.
#   Top-K filter (K=64 in our impl, paper 128).
#   ESU: MultiHeadAttention with candidate Q, top-K K/V → (B, D).
#
# Integration: per-domain TWINBlock × 4 → mean → Linear → LayerNorm →
# residual ADD post-backbone (output += sigmoid(twin_gate) * twin_state).
# Gate init sigmoid(-2.0)≈0.12 (§10.10). num_ns 변경 없음 → d_model%T 안전.


class TWINBlock(nn.Module):
    """Per-domain TWIN GSU+ESU retrieval module."""

    def __init__(self, d_model: int, num_heads: int = 4, top_k: int = 64) -> None:
        super().__init__()
        self.d_model = d_model
        self.top_k = top_k
    ...
    def forward(
        ...
        """
        Args:
            history: (B, L, d_model) per-domain seq embeddings.
            candidate: (B, d_model) candidate item aggregated representation.
            history_mask: (B, L) bool — True where padded.
        Returns:
            attended: (B, d_model) candidate-aware retrieved state.
        """
        B, L, D = history.shape
        K = min(self.top_k, L)
        # GSU — parameter-free inner product score.
        ...
        topk_pad_mask = (topk_scores == float('-inf'))  # (B, K)
        # Defensive: rows that are entirely padded would have all -inf scores
        # → MultiheadAttention would NaN. Force first slot un-masked in such
        # rows (its embedding is zero anyway → contributes nothing).
        all_padded = topk_pad_mask.all(dim=-1, keepdim=True)  # (B, 1)
        topk_pad_mask = topk_pad_mask & ~all_padded
        # ESU — candidate as query.
        candidate_q = candidate.unsqueeze(1)  # (B, 1, D)
        attended, _ = self.esu(
            candidate_q, topk_history, topk_history,
            key_padding_mask=topk_pad_mask,
        )
        attended = self.norm(candidate_q + attended).squeeze(1)  # (B, D)
        return attended


class TwinRetrievalAggregator(nn.Module):
    """Aggregates per-domain TWIN (B, D) outputs → (B, d_model) state + sigmoid gate."""

    def __init__(self, d_model: int, gate_init: float = -2.0) -> None:
        super().__init__()
        self.proj = nn.Linear(d_model, d_model)
        self.out_ln = nn.LayerNorm(d_model)
        self.twin_gate = nn.Parameter(torch.tensor(float(gate_init)))

    def forward(self, per_domain_states: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        stacked = torch.stack(per_domain_states, dim=1)  # (B, num_domains, D)
        pooled = stacked.mean(dim=1)  # (B, D)
        twin_state = self.out_ln(self.proj(pooled))
        gate = torch.sigmoid(self.twin_gate)
        return twin_state, gate
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H019 — TWIN long-seq retrieval (per-domain GSU+ESU, residual ADD post-backbone)
        use_twin_retrieval: bool = False,
        twin_top_k: int = 64,
        twin_num_heads: int = 4,
        twin_gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        ...
        # H019 — TWIN long-seq retrieval (per-domain GSU+ESU)
        self.use_twin_retrieval = use_twin_retrieval
        if use_twin_retrieval:
            ...
            logging.info(
                f"H019 TWIN retrieval enabled: top_k={twin_top_k} "
                f"num_heads={twin_num_heads} gate_init={twin_gate_init}"
            )
        else:
            self.twin_blocks = None
            self.twin_aggregator = None
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H019 — TWIN per-domain retrieval residual ADD
        if self.use_twin_retrieval:
            output = output + self._compute_twin_residual(inputs, seq_tokens_list, seq_masks_list)
    ...
    def _compute_twin_residual(
        self,
        inputs: ModelInput,
        seq_tokens_list: List[torch.Tensor],
        seq_masks_list: List[torch.Tensor],
    ) -> torch.Tensor:
        """H019: per-domain TWIN GSU+ESU → mean → gated residual."""
        # Candidate = mean of item NS tokens (re-tokenize is cheap).
        item_ns = self.item_ns_tokenizer(inputs.item_int_feats)  # (B, num_item_ns, D)
        candidate = item_ns.mean(dim=1)  # (B, D)
        per_domain_states: List[torch.Tensor] = []
        for i, domain_name in enumerate(self.seq_domains):
            twin_block = self.twin_blocks[f'twin_{domain_name}']
            twin_out = twin_block(
                seq_tokens_list[i], candidate, seq_masks_list[i]
            )
            per_domain_states.append(twin_out)
        twin_state, twin_gate = self.twin_aggregator(per_domain_states)
        return twin_gate * twin_state
