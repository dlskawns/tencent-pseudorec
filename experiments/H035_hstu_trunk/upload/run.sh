#!/bin/bash
# Platform run.sh for H035_hstu_trunk.
# Paradigm shift sub-H of H019 — per-domain sequence encoder 를 transformer
# (softmax self-attention) → HSTU (pointwise silu-attention + gated linear unit)
# 으로 trunk replacement. paper Meta 2024 ICML arXiv:2402.17152 paper-inspired.
# H019 의 모든 다른 부분 (TWIN GSU+ESU, NS xattn, DCN-V2, gate=-2.0, top_k=64,
# seq_max_lens 256/256/256/256, batch=1024) byte-identical.
# §17.4 retrieval_long_seq → backbone_replacement 카테고리 전환 (NEW first-touch
# auto-justified). data_ratio=1 결과 (0.837785 < 0.839674) 의 ceiling 신호 →
# capacity / mechanism class 자체가 bottleneck 일 가능성 → trunk 대체 강한 hedge.
#
...
echo "[run.sh] mode=H035 HSTU per-domain seq encoder (trunk replacement, paradigm shift)"
...
echo "[run.sh] training complete (H035); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'seq_encoder_type=hstu' indicator"
