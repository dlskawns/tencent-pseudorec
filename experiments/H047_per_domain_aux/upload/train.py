...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--use_per_domain_aux', action='store_true', default=False,
                        help='H047: enable 4 per-domain auxiliary classifier heads. '
                             'Each head predicts the same main label from its domain pooled seq. '
                             'Multi-task BCE forces backbone to encode balanced per-domain signal. '
                             'Aux weight 0.25 (4 heads × 0.25 = 1.0 vs main BCE).')
...
def main() -> None:
    ...
    model_args = {
        ...
        # H047 — per-domain auxiliary heads
        "use_per_domain_aux": args.use_per_domain_aux,
    }
