...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H047 — per-domain auxiliary classifier heads (multi-task BCE)
        use_per_domain_aux: bool = False,
    ) -> None:
        super().__init__()
        ...
        # H047 — per-domain auxiliary classifier heads (multi-task BCE on label)
        self.use_per_domain_aux = use_per_domain_aux
        if use_per_domain_aux:
            self.per_domain_aux_heads = nn.ModuleList([
                nn.Linear(d_model, 1) for _ in range(4)
            ])
            logging.info(
                f"H047 per-domain aux heads enabled: 4 heads, multi-task BCE on label"
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        H047: returns (logits, per_domain_aux_logits_list) tuple when
        use_per_domain_aux=True. Otherwise returns logits only (H019 byte-identical).
        """
        ...
        # H047 — per-domain auxiliary BCE heads on each domain's pooled seq.
        # Each head predicts the same main label → forces backbone to encode
        # balanced per-domain prediction signal.
        if self.use_per_domain_aux:
            per_domain_aux: List[torch.Tensor] = []
            for i in range(4):
                tokens = seq_tokens_list[i]
                mask = seq_masks_list[i]
                valid = (~mask).float().unsqueeze(-1)
                sum_valid = valid.sum(dim=1).clamp(min=1.0)
                pooled = (tokens * valid).sum(dim=1) / sum_valid  # (B, D)
                aux_logit = self.per_domain_aux_heads[i](pooled).squeeze(-1)  # (B,)
