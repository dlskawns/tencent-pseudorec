...
class PCVRHyFormerRankingTrainer:
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H047: model returns (logits, per_domain_aux_list) when use_per_domain_aux=True.
        if isinstance(model_out, tuple):
            raw_logits, per_domain_aux = model_out
        else:
        ...
        # H047 — per-domain aux BCE: each domain head predicts same main label.
        # λ=0.25 → 4 heads × 0.25 = 1.0 total weight (balanced with main BCE).
        if per_domain_aux is not None:
            aux_losses = [F.binary_cross_entropy_with_logits(a, label) for a in per_domain_aux]
