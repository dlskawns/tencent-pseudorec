...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H041 — cold-start dual branch (data signal: target_in_history=0.4%, 99.6% extrapolation)
        use_cold_start_branch: bool = False,
        cold_start_gate_init: float = 2.0,    # sigmoid(2)≈0.88 → main branch dominates initially
    ) -> None:
        super().__init__()
        ...
        # H041 — cold-start dual classifier branch (data signal: target_in_history=0.4%)
        self.use_cold_start_branch = use_cold_start_branch
        if use_cold_start_branch:
            self.cold_clsfier = nn.Sequential(
                nn.Linear(d_model, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
                nn.Dropout(dropout_rate),
            ...
            # Per-sample gate: sigmoid(W·output + b). Init b=cold_start_gate_init,
            # W=0 → initial gate = sigmoid(b) = sigmoid(2.0) ≈ 0.88 (main branch dominates).
            self.cold_gate = nn.Linear(d_model, 1)
            nn.init.zeros_(self.cold_gate.weight)
            nn.init.constant_(self.cold_gate.bias, float(cold_start_gate_init))
            logging.info(
                f"H041 cold-start dual branch enabled: gate_init bias={cold_start_gate_init} "
                f"(sigmoid({cold_start_gate_init})={torch.sigmoid(torch.tensor(cold_start_gate_init)).item():.3f} → main dominates)"
            )
        else:
            self.cold_clsfier = None
            self.cold_gate = None
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        logits = self._compute_classifier(output)  # H041 dispatch
        return logits
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_twin_retrieval:
            output = output + self._compute_twin_residual(inputs, seq_tokens_list, seq_masks_list)
        logits = self._compute_classifier(output)
        return logits, output

    def _compute_classifier(self, output: torch.Tensor) -> torch.Tensor:
        """H041: dispatch between single classifier (H019) and dual cold-start branch."""
        if self.use_cold_start_branch:
            gate = torch.sigmoid(self.cold_gate(output))                # (B, 1)
            out_main = self.clsfier(output)                              # (B, action_num)
            out_cold = self.cold_clsfier(output)                         # (B, action_num)
            return gate * out_main + (1.0 - gate) * out_cold             # gated combination
