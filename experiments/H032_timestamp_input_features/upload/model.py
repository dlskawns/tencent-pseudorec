...
class ModelInput(NamedTuple):
    ...
    # H032 — timestamp-derived input features (Optional defaults preserve
    # backward-compat with H010-H031 callers; populated only when
    # --use_timestamp_features flag is on)
    hour_of_day: Optional[torch.Tensor] = None       # (B,) int64, 0-23
    day_of_week: Optional[torch.Tensor] = None       # (B,) int64, 0-6
    recency_bucket: Optional[torch.Tensor] = None    # (B,) int64, 0-7
...
# ═══════════════════════════════════════════════════════════════════════════════
# H032 — TimeFeaturesBlock (timestamp-derived input features, EDA-driven)
# ═══════════════════════════════════════════════════════════════════════════════
# Motivation (eda/out/timestamp_signal_audit.json):
#   model.py 의 timestamp/label_time keyword zero-hits → 9 H 모두 timestamp
#   를 input feature 로 사용 안 함. dataset.py 가 batch dict 에 timestamp 만
#   포함, ModelInput NamedTuple 에는 빠짐. seq 내부 time bucketing 에만
#   timestamp 사용. H015~H018 의 recency 시도는 모두 loss reweight axis →
#   본 H032 가 input feature axis 첫 진입.
#
# Mechanism: 3 derived categorical embeddings (hour=24, dow=7, recency=8)
# 각 16-dim → sum → LayerNorm → Linear → (B, d_model) time_state. Residual
# ADD post-backbone: output += sigmoid(gate) * time_state. Gate init = -2.0
# (sigmoid ≈ 0.12) per CLAUDE.md §10.10 InterFormer mandate. num_ns 변경
# 없음 → d_model % T 제약 안전.


class TimeFeaturesBlock(nn.Module):
    """Timestamp-derived input features → (B, d_model) state + sigmoid gate.

    Inputs:
      hour_of_day:   (B,) int64, 0-23
      day_of_week:   (B,) int64, 0-6
      recency_bucket:(B,) int64, 0-7

    Output:
      time_state: (B, d_model)
      gate:       () scalar, sigmoid(cross_gate)
    """
    ...
    def __init__(
        ...
        num_dow_buckets: int = 7,
        num_recency_buckets: int = 8,
        gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        self.hour_emb = nn.Embedding(num_hour_buckets, time_emb_dim)
        self.dow_emb = nn.Embedding(num_dow_buckets, time_emb_dim)
        self.recency_emb = nn.Embedding(num_recency_buckets, time_emb_dim)
        self.time_ln = nn.LayerNorm(time_emb_dim)
        self.time_proj = nn.Linear(time_emb_dim, d_model)
        self.out_ln = nn.LayerNorm(d_model)
        self.time_gate = nn.Parameter(torch.tensor(float(gate_init)))
        nn.init.xavier_normal_(self.hour_emb.weight.data)
        nn.init.xavier_normal_(self.dow_emb.weight.data)
        nn.init.xavier_normal_(self.recency_emb.weight.data)
    ...
    def forward(
        ...
        # Defensive clamps — input pre-clipped at dataset side, but guard against
        # any unexpected OOB at inference time (timestamp.fill_null(0) → bucket 0).
        h = hour_of_day.long().clamp_(0, self.hour_emb.num_embeddings - 1)
        d = day_of_week.long().clamp_(0, self.dow_emb.num_embeddings - 1)
        r = recency_bucket.long().clamp_(0, self.recency_emb.num_embeddings - 1)
        time_emb = self.hour_emb(h) + self.dow_emb(d) + self.recency_emb(r)
        time_emb = self.time_ln(time_emb)
        time_state = self.out_ln(self.time_proj(time_emb))
        gate = torch.sigmoid(self.time_gate)
        return time_state, gate
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # H032 — Timestamp-derived input features (residual ADD post-backbone)
        use_timestamp_features: bool = False,
        time_emb_dim: int = 16,
        time_gate_init: float = -2.0,
    ) -> None:
        super().__init__()
        ...
        # H032 — Timestamp-derived input features block
        self.use_timestamp_features = use_timestamp_features
        if use_timestamp_features:
            self.time_features_block = TimeFeaturesBlock(
                d_model=d_model,
                time_emb_dim=time_emb_dim,
                gate_init=time_gate_init,
            )
            logging.info(
                f"H032 TimeFeaturesBlock enabled: time_emb_dim={time_emb_dim} "
                f"gate_init={time_gate_init} (residual ADD post-backbone)"
            )
        else:
            self.time_features_block = None
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # H032 — residual ADD of timestamp-derived state (gated, init ≈ 0.12)
        if self.time_features_block is not None:
            assert inputs.hour_of_day is not None, \
                "H032 use_timestamp_features=True but inputs.hour_of_day is None"
            time_state, time_gate = self.time_features_block(
                inputs.hour_of_day, inputs.day_of_week, inputs.recency_bucket
            )
            output = output + time_gate * time_state
