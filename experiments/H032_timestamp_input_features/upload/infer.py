...
def _try_torch_inference(ckpt_dir: str, eval_files: list[str]):
    ...
    model = PCVRHyFormer(
        ...
        use_timestamp_features=cfg.get("use_timestamp_features", False),
        time_emb_dim=cfg.get("time_emb_dim", 16),
        time_gate_init=cfg.get("time_gate_init", -2.0),
    ).to(device)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            mi = ModelInput(
                ...
                # H032 — timestamp-derived input features (None if not produced)
                hour_of_day=batch["hour_of_day"].to(device) if "hour_of_day" in batch else None,
                day_of_week=batch["day_of_week"].to(device) if "day_of_week" in batch else None,
                recency_bucket=batch["recency_bucket"].to(device) if "recency_bucket" in batch else None,
            )
