...
class PCVRHyFormerRankingTrainer:
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        return ModelInput(
            ...
            # H032 — timestamp-derived input features (passed through if dataset
            # populated them; otherwise None and model.forward skips time block)
            hour_of_day=device_batch.get('hour_of_day'),
            day_of_week=device_batch.get('day_of_week'),
            recency_bucket=device_batch.get('recency_bucket'),
        )
