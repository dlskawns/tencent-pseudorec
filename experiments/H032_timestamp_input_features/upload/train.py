...
def parse_args() -> argparse.Namespace:
    ...
    # H032 — Timestamp-derived input features
    parser.add_argument('--use_timestamp_features', action='store_true', default=False,
                        help='H032: enable TimeFeaturesBlock — 3 derived categorical '
                             '(hour_of_day=24, day_of_week=7, recency_bucket=8 from '
                             'log2(timestamp - max_seq_ts)) → 16-dim emb sum → LN → '
                             'Linear → (B, d_model) → residual ADD post-backbone. '
                             'Gate init = sigmoid(-2) ≈ 0.12 per CLAUDE.md §10.10. '
                             'EDA: eda/out/timestamp_signal_audit.json.')
    parser.add_argument('--time_emb_dim', type=int, default=16,
                        help='H032: per-feature time embedding dim. Effective with --use_timestamp_features.')
    parser.add_argument('--time_gate_init', type=float, default=-2.0,
                        help='H032: time_gate init (sigmoid(-2.0) ≈ 0.12 per InterFormer §10.10).')
...
def main() -> None:
    ...
    model_args = {
        ...
        # H032 — Timestamp-derived input features
        "use_timestamp_features": args.use_timestamp_features,
        "time_emb_dim": args.time_emb_dim,
        "time_gate_init": args.time_gate_init,
    }
