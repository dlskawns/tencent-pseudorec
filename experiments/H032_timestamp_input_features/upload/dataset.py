...
class PCVRParquetDataset(IterableDataset):
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ---- meta ----
        # H032 — §18.7 defensive: fill_null(0) guards inference data where
        # timestamp may be null. Without this, ArrowInvalid is raised by
        # to_numpy(zero_copy_only=True default). Aligns with H015/H017
        # label_time bug fix pattern.
        timestamps = (batch.column(self._col_idx['timestamp']).fill_null(0)
        ...
        # ---- H032 — per-row max seq timestamp tracker (across all domains) ----
        # Used downstream to compute recency_bucket = log2(timestamp - max_seq_ts).
        # Updated inside the seq loop where ts_padded is already materialized.
        max_seq_ts_global = np.zeros(B, dtype=np.int64)
        ...
        for domain in self.seq_domains:
            ...
            if ts_ci is not None:
                ...
                # H032 — track per-row max ts across all domains for recency feature
                np.maximum(max_seq_ts_global, ts_padded.max(axis=1), out=max_seq_ts_global)
        ...
        # hour_of_day: (timestamp // 3600) % 24 — 24 buckets
        # day_of_week: (timestamp // 86400) % 7 — 7 buckets
        # recency_bucket: clip(0, 7, log2(max(1, timestamp - max_seq_ts))) — 8 buckets
        # Defensive: timestamps may be 0 at inference (§18.7 fill_null pattern) →
        # all derived features become bucket 0 (default), no IndexError downstream.
        ts_safe = np.maximum(timestamps, 0)
        hour = ((ts_safe // 3600) % 24).astype(np.int64)
        dow = ((ts_safe // 86400) % 7).astype(np.int64)
        # recency: positive elapsed seconds since last seq event
        recency_diff = np.maximum(ts_safe - max_seq_ts_global, 1)
        log2_recency = np.log2(recency_diff.astype(np.float64))
        recency = np.clip(log2_recency.astype(np.int64), 0, 7)
        # If max_seq_ts_global is 0 (no seq history at all), force recency = 0.
        recency = np.where(max_seq_ts_global == 0, 0, recency).astype(np.int64)
        result['hour_of_day'] = torch.from_numpy(hour.copy())
        result['day_of_week'] = torch.from_numpy(dow.copy())
        result['recency_bucket'] = torch.from_numpy(recency.copy())
