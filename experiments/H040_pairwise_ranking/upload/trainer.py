...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        bpr_num_neg: int = 4,    # H040: negatives per positive for BPR pairwise
        ...
        self.bpr_num_neg: int = bpr_num_neg    # H040
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        if self.loss_type == 'bpr':
            # H040: pairwise BPR ranking loss — directly optimizes AUC.
            # For each positive in batch, sample K random negatives.
            # loss = -log(sigmoid(z_pos - z_neg)) averaged over (positive, negative) pairs.
            # Fallback to BCE if batch has no positives or no negatives (rare with batch=1024).
            pos_mask = label > 0.5
            neg_mask = ~pos_mask
            ...
            if num_pos == 0 or num_neg == 0:
                ...
                k = min(self.bpr_num_neg, num_neg)
                # Sample K negatives per positive (with replacement).
                neg_idx = torch.randint(
                    0, num_neg, (num_pos, k), device=logits.device
                )
                sampled_neg = neg_logits[neg_idx]                       # (P, K)
                diff = pos_logits.unsqueeze(1) - sampled_neg            # (P, K)
