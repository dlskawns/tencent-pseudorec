#!/bin/bash
# Platform run.sh for H040_pairwise_ranking.
# Data-signal-driven (CLAUDE.md §0.5, R4 reframe): platform metric = AUC =
# ranking quality. 18 H 모두 pointwise BCE 사용. BCE 는 ranking 직접 최적화 안 함.
# Mutation: BCE → BPR pairwise — for each positive, sample K random negatives
# from same batch, compute -log(sigmoid(z_pos - z_neg)). AUC 직접 최적화.
...
export EXP_ID="H040_pairwise_ranking"
...
echo "[run.sh] mode=H040 BPR pairwise ranking (R4 reframe — AUC direct optimization)"
...
echo "[run.sh] training complete (H040); metrics at ${TRAIN_CKPT_PATH}/metrics.json"
echo "[run.sh] check stdout above for §18.8 SUMMARY block + 'loss_type=bpr bpr_num_neg=4' indicator"
