...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        lambdarank_lambda: float = 0.0,    # H051 — 0.0 → off (H019 byte-identical)
        ...
        self.lambdarank_lambda: float = lambdarank_lambda    # H051
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        # H051 — LambdaRank-lite listwise: pairwise (pos, neg) loss weighted by
        # |Δ NDCG| approximation (sigmoid(-(z_pos - z_neg)) gives higher weight
        # for wrong-ordered pairs = where neg > pos). H040 BPR REFUTED used uniform
        # weighting — H051 = lambda-weighted (focus on hard pairs). Direct AUC
        # ranking optimization (NEW objective form, 14 H 동안 listwise 0회).
        if self.lambdarank_lambda > 0:
            ...
            if pos_mask.sum() > 0 and neg_mask.sum() > 0:
                ...
                # Lambda weight (no grad on weight): sigmoid(-diff) = high for misranked
                weight = torch.sigmoid(-diff.detach())      # (P, N)
                pair_loss = -F.logsigmoid(diff) * weight    # (P, N)
                listwise_loss = pair_loss.mean()
                loss = loss + self.lambdarank_lambda * listwise_loss
