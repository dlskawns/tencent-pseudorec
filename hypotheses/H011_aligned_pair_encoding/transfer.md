...
- **신규 module / 변경**:
  1. `RankMixerNSTokenizer.__init__` 에 `aligned_dense_specs: Optional[Dict[int,
     Tuple[int, int]]] = None` (fid_idx → (dense_offset, dense_dim) 매핑) 인자.
  2. `RankMixerNSTokenizer.forward` 가 두 번째 optional arg `dense_feats`
     받음. aligned fid k 의 mean-pool 단계에서 **per-row L1-normalized weighted
     mean** 적용 (Option α — gap #3 audit 결과 mandated):
     ```python
     ...
     ```
  3. non-aligned fids (38 user_int + dense-only `{61, 87}`) 변경 없음.
     `user_dense_proj` 도 그대로 (전체 dense flat 입력 받아 단일 NS token).
  4. **dense feature scale handling 결정 근거** (출처: `eda/out/dense_value_stats.json`):
     - **Pattern X (fid 62-66)**: unbounded count/duration, max=4.8M~18.4M.
       raw multiply 시 gradient explode 확정 → normalize mandatory.
     - **Pattern Y (fid 89-91)**: ~[-1, +1] 범위, std≈0.32, 60% negative.
       raw multiply 가능하지만 Pattern X 와 통일된 처리 위해 같은 normalize.
     - **Option α (per-row L1 normalize) 선택 이유**: parameter-free, 두
       pattern 통일, sign 보존, NaN 안전 (clamp), baseline mean-pool 의
       자연스러운 weighted generalization (uniform weights → 정확히 baseline 으로 reduce).
